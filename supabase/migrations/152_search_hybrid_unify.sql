-- Migration 152: make search_listings_hybrid the single canonical retrieval
-- function. Adds geo bbox, exclude-vertical/suburb, min-quality, trail-suitable,
-- and cross-vertical (verticals[]) matching, plus a superset return so
-- plan/itinerary/similar can all call it. Backward-compatible: every new arg is
-- defaulted, so the front-door and vibe callers (which pass a subset) are
-- unaffected. The return type grows, so the function is dropped and recreated.
...
create or replace function search_listings_hybrid(
  ...
  similarity_floor float default 0.48,  -- calibrated Phase 7
...
returns table (
  ...
  region text, state text, suburb text, lat float8, lng float8,
...
-- ef_search stays at pgvector's default 40 (calibrated sufficient). The pooled
-- role can't SET the GUC at definition time; raise it at the session/role level.
as $$
declare
...
begin
  ...
  select b.id, b.name, b.slug, b.vertical, b.sub_type, b.description,
         b.region, b.state, b.suburb, b.lat, b.lng,
