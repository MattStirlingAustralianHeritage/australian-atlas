...
-- (lib/sync/pushToVertical.js). Adds 'confectioner' — traditional sweet makers
-- (fudge, nougat, toffee, brittle, boiled lollies) distinct from chocolatier.
...
ALTER TABLE table_meta ADD CONSTRAINT table_meta_food_type_check
  CHECK (food_type IN (
    ...
    'creamery', 'chocolatier', 'confectioner'
  ));
