...
const EXAMPLE_TRAILS = [
  ...
  { query: 'Byron hinterland three-day stay with farm stays', region: 'Byron Hinterland, NSW', days: '3 days', stops: '10 stops', verticals: ['Rest', 'Table', 'Field'] },
    ...
    <div style={{ background: 'var(--color-bg)', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'var(--color-cream)', borderBottom: '1px solid var(--color-border)' }}>
        <div className="max-w-4xl mx-auto text-center" style={{ padding: '64px 24px 56px' }}>
          <p className="section-dateline" style={{ marginBottom: 14 }}>{t('discoveryTrails')}</p>
          ...
          <TrailPromptInput placeholder={t('promptPlaceholder')} buildLabel={t('buildTrail')} />
      ...
      <div className="max-w-6xl mx-auto px-6" style={{ paddingTop: 48 }}>
        <div style={{ marginBottom: 24 }}>
          <p style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--color-sage)', fontFamily: 'var(--font-body)', fontWeight: 600, marginBottom: 4 }}>{t('trySomethingLike')}</p>
          <p style={{ fontSize: 13, color: 'var(--color-muted)', fontFamily: 'var(--font-body)' }}>{t('clickExample')}</p>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-6" style={{ paddingTop: 56 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <p style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--color-sage)', fontFamily: 'var(--font-body)', fontWeight: 600, marginBottom: 4 }}>{t('editorialTrails')}</p>
        ...
        {(!editorialTrails || editorialTrails.length === 0) ? (
          <div style={{ textAlign: 'center', padding: '48px 0', color: 'var(--color-muted)', fontFamily: 'var(--font-body)', fontSize: 14 }}>
            {t('editorialComingSoon')}
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 24 }}>
            {editorialTrails.map(trail => (
              <EditorialTrailCard key={trail.id} trail={trail} t={t} />
            ))}
          </div>
        )}
      </div>
      ...
      {communityTrails && communityTrails.length > 0 && (
        <div className="max-w-6xl mx-auto px-6" style={{ paddingTop: 48 }}>
          <div style={{ marginBottom: 24 }}>
            <p style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--color-sage)', fontFamily: 'var(--font-body)', fontWeight: 600, marginBottom: 4 }}>{t('communityTrails')}</p>
            <p style={{ fontSize: 13, color: 'var(--color-muted)', fontFamily: 'var(--font-body)' }}>{t('builtByReaders')}</p>
          </div>
              ...
              return (
                <Link key={trail.id} href={href} style={{ textDecoration: 'none', display: 'block', background: 'var(--color-card-bg)', border: '1px solid var(--color-border)', borderRadius: 4, padding: '16px 18px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--color-muted)', fontFamily: 'var(--font-body)' }}>
                    <span>{t('stopCount', { count: trail.stop_count || 0 })}</span>
      ...
      <div className="max-w-6xl mx-auto px-6" style={{ padding: '56px 24px 80px' }}>
        <div style={{ borderTop: '1px solid var(--color-border)', paddingTop: 48 }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <p style={{ fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--color-sage)', fontFamily: 'var(--font-body)', fontWeight: 600, marginBottom: 8 }}>{t('howItWorks')}</p>
          </div>
...
function EditorialTrailCard({ trail, t }) {
  return (
      ...
      <div style={{ background: 'var(--color-card-bg)', borderRadius: 4, overflow: 'hidden', border: '1px solid var(--color-border)', transition: 'border-color 0.2s' }}>
        <div style={{
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 8, fontWeight: 500, letterSpacing: '0.14em', textTransform: 'uppercase', opacity: 0.55, margin: '0 0 1rem' }}>{t('discoveryTrailKicker')}</p>
        ...
        <div style={{ padding: '20px 22px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 11, color: 'var(--color-muted)', fontFamily: 'var(--font-body)' }}>{t('stopCount', { count: trail.stop_count || 0 })}</span>
            {trail.vertical_focus && (
              <>
          ...
          {trail.curator_name && (
            <p style={{ marginTop: 12, fontSize: 11, color: 'var(--color-muted)', fontFamily: 'var(--font-body)' }}>{t('curatedBy', { name: trail.curator_name })}</p>
          )}
        </div>
      </div>
    </Link>
