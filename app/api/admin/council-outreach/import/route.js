...
/**
 * POST /api/admin/council-outreach/import
 * Bulk-import councils into the directory (CSV parsed client-side).
 *
 * Body: { rows: [{ council_name, state?, website?, contact_email?, region_slug?, region_name?, covers?, contact_name?, contact_role? }] }
 * Dedup: case-insensitive (council_name, state) against existing rows and
...
export async function POST(request) {
  ...
  const { data: existingRows, error: exErr } = await sb
    ...
    .select('council_name, state')
    .limit(5000)
  if (exErr) return NextResponse.json({ error: exErr.message }, { status: 500 })
  const seen = new Set((existingRows || []).map((r) => `${(r.council_name || '').toLowerCase()}|${r.state || ''}`))
  ...
  for (const raw of rows) {
    const name = (raw.council_name || '').trim()
  ...
  let inserted = 0
  if (toInsert.length) {
    const { error: insErr } = await sb.from('council_outreach').insert(toInsert)
    if (insErr) {
      console.error('[council-outreach/import] insert error:', insErr.message)
