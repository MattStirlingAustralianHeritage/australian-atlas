...
/**
 * POST /api/admin/council-outreach
 * Add one council to the outreach directory.
 * Body: { council_name, state?, website?, contact_email?, contact_name?, contact_role?, region_slug?, region_name?, covers?, notes? }
 */
export async function POST(request) {
  ...
  const name = (body.council_name || '').trim()
  if (!name) {
    return NextResponse.json({ error: 'council_name is required' }, { status: 400 })
  }
  ...
  const { data: existing } = await sb
    ...
    .select('id, council_name, state')
    .ilike('council_name', name)
  if ((existing || []).some((r) => (r.state || '') === (state || ''))) {
    return NextResponse.json({ error: 'That council is already in the directory' }, { status: 409 })
  }
  ...
  if (error) {
    ...
    console.error('[council-outreach POST] Error:', error.message)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }

  return NextResponse.json({ ok: true, council: data })
}

/**
 * PATCH /api/admin/council-outreach
 * Update a directory row.
 * Body: { id, status?, notes?, contact_email?, contact_name?, contact_role?, website?, region_slug? }
 */
export async function PATCH(request) {
  ...
  if (error) {
    console.error('[council-outreach PATCH] Error:', error.message)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
