...
/**
 * POST /api/admin/trade-outreach/segment
 * Resolve a trade recipient segment for the compose UI, enriched with
 * outreach state (email, suppression, prior send) and the joined focus region.
 *
 * Body: { state?, org_type?, q?, status?, limit? }
 */
export async function POST(request) {
  ...
  let query = sb
    .from('trade_outreach')
    .select('id, company_name, org_type, state, website, region_id, region_name, focus, contact_name, contact_role, contact_email, email_source, discovered_at, personal_note, status, send_status, sent_at, campaign_id, last_contacted_at, notes, regions:region_id (id, name, slug, state, listing_count)')
  ...
  if (q) query = query.or(`company_name.ilike.%${q}%,region_name.ilike.%${q}%,focus.ilike.%${q}%`)
  ...
  const enriched = (rows || []).map((r) => {
    ...
    // Same semantics as operator/council outreach: when an email is present
  ...
  return NextResponse.json({ ok: true, companies: enriched, counts })
}
