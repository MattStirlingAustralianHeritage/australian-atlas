...
import { INDUSTRY_TEMPLATE_OPTIONS, INDUSTRY_TEMPLATES, INDUSTRY_INTRO_TEMPLATE } from '@/lib/outreach/industryTemplates'
...
const MERGE_TOKENS = [
  ...
  ['{{about_url}}', '/about URL'],
  ['{{venues_url}}', '/for-venues URL'],
  ['{{regions_url}}', '/regions URL'],
]

const ORG_TYPE_OPTIONS = [
  ['peak_body', 'Peak body'],
  ['association', 'Association'],
  ['tourism_org', 'Tourism organisation'],
...
// Mirror buildIndustryMergeContext (lib/outreach/industryTemplate.js) so the
// live preview matches what the send route renders.
function focusPhrase(focus) {
...
function buildCtx(c, noteOverride) {
  return {
    greeting_name: firstName(c.contact_name) || 'there',
    org_name: c.org_name || 'your organisation',
    contact_name: c.contact_name || '',
    focus: focusPhrase(c.focus),
    ...
    venues_url: `${SITE}/for-venues`,
    regions_url: `${SITE}/regions`,
  }
}
...
// HTML, auth redirects). Same as the operator/council/press outreach UI.
  ...
  return (
    <div style={{ marginBottom: 22 }}>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 8, flexWrap: 'wrap' }}>
        {[
          ['named contacts', stats.contacts], ['organisations', stats.orgs], ['suppressed', stats.suppressed],
...
function displayName(c) {
  return c.contact_name ? c.contact_name : c.org_name
}

// ── Compose & Send panel ──────────────────────────────────────
function ComposePanel({ statusColors, sendStatusColors, allStates }) {
  const [filters, setFilters] = useState({ kind: '', org_type: '', state: '', focus: '', q: '', status: '', limit: 200 })
  const [loading, setLoading] = useState(false)
  const [seg, setSeg] = useState(null) // { orgs, counts }
  ...
  const [templateChoice, setTemplateChoice] = useState('intro')
  const [subject, setSubject] = useState(INDUSTRY_INTRO_TEMPLATE.subject)
  const [emailBody, setEmailBody] = useState(INDUSTRY_INTRO_TEMPLATE.body)
  ...
  function loadTemplate(choice) {
    ...
    const t = INDUSTRY_TEMPLATES[choice] || INDUSTRY_INTRO_TEMPLATE
  ...
  async function loadSegment() {
    ...
    try {
      const data = await fetchJson('/api/admin/industry-outreach/segment', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(filters),
      })
      setSeg(data)
      setSelected(new Set((data.orgs || []).filter((c) => c.sendable).map((c) => c.id)))
  ...
  const selectAllSendable = () => seg && setSelected(new Set(seg.orgs.filter((c) => c.sendable).map((c) => c.id)))
  const clearSelection = () => setSelected(new Set())

  function recomputeCounts(orgs) {
    const un = new Set(['sent', 'bounced', 'complained', 'unsubscribed'])
    return {
      total: orgs.length,
      contacts: orgs.filter((c) => c.kind === 'contact').length,
      orgs: orgs.filter((c) => c.kind === 'org').length,
      withWebsite: orgs.filter((c) => c.website).length,
      withEmail: orgs.filter((c) => c.contact_email).length,
      suppressed: orgs.filter((c) => c.suppressed).length,
      alreadySent: orgs.filter((c) => un.has(c.send_status)).length,
      sendable: orgs.filter((c) => c.sendable).length,
    }
  }
  ...
  async function discoverEmails() {
    if (!seg) return
    const needing = needDiscover(seg.orgs)
    if (needing.length === 0) return
    setDiscovering(true); setError(null); setDiscoverSummary(null)
    const chunkSize = 10
    let scanned = 0, found = 0
    const updated = new Map(seg.orgs.map((c) => [c.id, c]))
    const failures = []
    ...
    try {
      for (let i = 0; i < needing.length; i += chunkSize) {
        ...
        try {
          const data = await fetchJson('/api/admin/industry-outreach/discover', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ industry_ids: chunk.map((c) => c.id) }),
          })
          for (const r of data.results || []) {
            const c = updated.get(r.industry_id)
            if (!c) continue
            if (r.email && r.status === 'found') {
              updated.set(r.industry_id, { ...c, contact_email: r.email, email_source: 'website', website_status: 'has_email', sendable: !c.suppressed && !['sent', 'bounced', 'complained', 'unsubscribed'].includes(c.send_status) && c.funnel_status !== 'declined' })
              found++
            } else if (r.status && r.status !== 'pending' && r.status !== 'has_email') {
              updated.set(r.industry_id, { ...c, website_status: r.status })
            }
          }
      ...
      const orgs = seg.orgs.map((c) => updated.get(c.id))
      setSeg({ orgs, counts: recomputeCounts(orgs) })
      setSelected((prev) => {
        const next = new Set(prev)
        for (const c of orgs) if (c.sendable && c.contact_email) next.add(c.id)
        return next
      })
  ...
  async function personaliseSelected() {
    if (!seg) return
    const targets = seg.orgs.filter((c) => selected.has(c.id) && c.sendable && !c.personal_note)
    if (targets.length === 0) return
    setWriting(true); setError(null)
    const chunkSize = 12
    let done = 0, wrote = 0
    const byId = new Map(seg.orgs.map((c) => [c.id, c]))
    const failures = []
    try {
      for (let i = 0; i < targets.length; i += chunkSize) {
        ...
        try {
          const data = await fetchJson('/api/admin/industry-outreach/personalise', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ industry_ids: chunk.map((c) => c.id) }),
          })
          for (const r of data.results || []) {
            const c = byId.get(r.industry_id)
            if (c && r.personal_note) { byId.set(r.industry_id, { ...c, personal_note: r.personal_note }); wrote++ }
          }
        } catch (err) { failures.push(err.message) }
        done += chunk.length
        setPersonaliseProgress({ done, total: targets.length })
      }
      setSeg((prev) => prev && { ...prev, orgs: prev.orgs.map((c) => byId.get(c.id)) })
      if (failures.length) {
  ...
  function updateNote(id, note) {
    setSeg((prev) => prev && { ...prev, orgs: prev.orgs.map((c) => c.id === id ? { ...c, personal_note: note } : c) })
  }

  const eligibleSelected = useMemo(() => {
    if (!seg) return []
    return seg.orgs.filter((c) => selected.has(c.id) && c.sendable && c.contact_email)
  }, [seg, selected])

  const notedCount = useMemo(() => eligibleSelected.filter((c) => c.personal_note).length, [eligibleSelected])
  const [previewId, setPreviewId] = useState(null)
  const previewRow = (previewId && (seg?.orgs || []).find((c) => c.id === previewId)) || eligibleSelected[0] || (seg?.orgs || [])[0] || null
  ...
  async function runSend({ dryRun = false, testMode = false } = {}) {
    ...
    try {
      const data = await fetchJson('/api/admin/industry-outreach/send', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          industry_ids: eligibleSelected.map((c) => c.id),
      ...
      if (!dryRun && !testMode) {
        setSeg((prev) => prev && {
          ...prev,
          orgs: prev.orgs.map((c) => eligibleSelected.find((e) => e.id === c.id) ? { ...c, send_status: 'sent', sendable: false, funnel_status: 'contacted' } : c),
          counts: prev.counts,
        })
        setSelected(new Set())
      }
  ...
  const discoverCount = seg ? needDiscover(seg.orgs).length : 0
  const personaliseCount = seg ? seg.orgs.filter((c) => selected.has(c.id) && c.sendable && !c.personal_note).length : 0
        ...
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'flex-end' }}>
          <div>
            ...
            <select style={input} value={filters.kind} onChange={(e) => setFilters({ ...filters, kind: e.target.value })}>
              ...
              <option value="contact">Named contacts</option>
              <option value="org">Organisations</option>
            </select>
          </div>
          <div>
            <label style={label}>Org type</label>
          ...
          <div>
            ...
            <input style={{ ...input, width: 100 }} value={filters.focus} onChange={(e) => setFilters({ ...filters, focus: e.target.value })} placeholder="e.g. wine" />
          </div>
          <div>
            <label style={label}>Name / org contains</label>
            <input style={{ ...input, width: 140 }} value={filters.q} onChange={(e) => setFilters({ ...filters, q: e.target.value })} placeholder="e.g. Tourism" />
          </div>
          <div>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', marginBottom: 14 }}>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button style={btn} onClick={discoverEmails} disabled={discovering || discoverCount === 0} title="Scan org/contact pages we haven't checked yet for a contact email">
                ...
                <div key={c.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={() => setPreviewId(c.id)} title="Preview this recipient">
                    <div style={{ fontFamily: 'var(--font-body, system-ui)', fontSize: 13, fontWeight: 500, color: previewRow && previewRow.id === c.id ? '#8a6520' : 'var(--color-ink, #2D2A26)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {displayName(c)}
                      <Chip color={c.kind === 'org' ? '#7a6f5f' : '#8a6520'}>{c.kind}</Chip>
                    </div>
                    <div style={{ fontFamily: 'var(--font-body, system-ui)', fontSize: 11, color: 'var(--color-muted, #888)' }}>
                      {[c.contact_name ? c.org_name : c.role_title, (c.focus || []).join(' / '), c.state].filter(Boolean).join(' · ')}
                    </div>
                  </div>
                  {(() => {
                    let text, color
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
            <div>
              ...
              <select style={{ ...input, width: '100%', marginBottom: 12 }} value={templateChoice} onChange={(e) => loadTemplate(e.target.value)}>
                {INDUSTRY_TEMPLATE_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
                ...
                {previewCtx ? (
                  <>
                    ...
                    <div style={{ marginTop: 16, paddingTop: 12, borderTop: '1px solid #eee', fontFamily: 'var(--font-body, system-ui)', fontSize: 11, color: '#8a8378', lineHeight: 1.6 }}>
                      ...
                      You received this because your organisation works with {previewCtx.focus} — we reach out to Australian industry bodies and organisations the Atlas may be useful to.<br />
          ...
          <div style={{ ...card, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
            ...
            <div style={{ fontFamily: 'var(--font-body, system-ui)', fontSize: 12, color: 'var(--color-muted, #888)', maxWidth: 280, lineHeight: 1.5 }}>
              Sends to the lesser of your selection and the cap. Emails include a working unsubscribe, go out from matt@australianatlas.com.au, and replies come straight back to Matt.
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 28, maxWidth: 460, width: '100%' }}>
            ...
            <p style={{ fontFamily: 'var(--font-body, system-ui)', fontSize: 14, color: '#5a544c', lineHeight: 1.5, margin: '0 0 20px' }}>
              About to send <strong>{Math.min(eligibleSelected.length, Number(cap) || 0)}</strong> real emails to industry contacts from matt@australianatlas.com.au. Recipients who have been contacted, suppressed, or unsubscribed are automatically excluded. This can&apos;t be undone.
            </p>
...
function DirectoryPanel({ regions, allStates }) {
  const [form, setForm] = useState({ kind: 'org', org_name: '', contact_name: '', role_title: '', org_type: '', focus: '', state: '', website: '', contact_email: '', linkedin: '', region_slug: '' })
  ...
  async function addOrg() {
    if (!form.org_name.trim()) { setAddMsg({ err: true, text: 'Organisation name is required.' }); return }
    setAdding(true); setAddMsg(null)
    try {
      await fetchJson('/api/admin/industry-outreach', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(form),
      })
      setAddMsg({ err: false, text: `${form.contact_name || form.org_name} added. Load a segment in Compose & Send to reach them.` })
      setForm({ kind: 'org', org_name: '', contact_name: '', role_title: '', org_type: '', focus: '', state: '', website: '', contact_email: '', linkedin: '', region_slug: '' })
    } catch (err) { setAddMsg({ err: true, text: err.message }) } finally { setAdding(false) }
  }

  async function runImport() {
    const { header, rows } = parseCsv(csvText)
    if (!header.includes('org_name')) {
      setImportMsg({ err: true, text: 'Need a header row with at least org_name — e.g. org_name,contact_name,role_title,org_type,focus,state,website,contact_email,region_slug,kind' })
      return
    }
    if (rows.length === 0) { setImportMsg({ err: true, text: 'No data rows found.' }); return }
    setImporting(true); setImportMsg(null)
    try {
      const data = await fetchJson('/api/admin/industry-outreach/import', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rows }),
      })
      const bits = [`${data.inserted} added`]
      if (data.skippedDuplicate) bits.push(`${data.skippedDuplicate} already in the directory`)
      if (data.skippedInvalid) bits.push(`${data.skippedInvalid} missing an organisation`)
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
      ...
      <div style={{ ...card, padding: 18 }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
          <div style={{ width: 130 }}>
            ...
            <select style={{ ...input, width: '100%' }} value={form.kind} onChange={(e) => set('kind', e.target.value)}>
              ...
              <option value="contact">Named contact</option>
            </select>
          </div>
          <div style={{ flex: 1 }}>
            <label style={label}>Organisation *</label>
            <input style={{ ...input, width: '100%' }} value={form.org_name} onChange={(e) => set('org_name', e.target.value)} placeholder="e.g. Tourism Industry Council Tasmania" />
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
          <div style={{ flex: 1 }}>
            <label style={label}>Contact name {form.kind === 'org' ? '(optional)' : ''}</label>
            <input style={{ ...input, width: '100%' }} value={form.contact_name} onChange={(e) => set('contact_name', e.target.value)} placeholder="e.g. Jane Smith" />
          </div>
          <div style={{ flex: 1 }}>
            <label style={label}>Role / title</label>
            <input style={{ ...input, width: '100%' }} value={form.role_title} onChange={(e) => set('role_title', e.target.value)} placeholder="e.g. CEO / Membership Manager" />
          </div>
        </div>
        <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
          <div style={{ width: 170 }}>
            <label style={label}>Org type</label>
            <select style={{ ...input, width: '100%' }} value={form.org_type} onChange={(e) => set('org_type', e.target.value)}>
              <option value="">—</option>
              {ORG_TYPE_OPTIONS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
            </select>
          </div>
          <div style={{ flex: 1 }}>
            <label style={label}>Focus (comma-separated)</label>
            <input style={{ ...input, width: '100%' }} value={form.focus} onChange={(e) => set('focus', e.target.value)} placeholder="wine, tourism, hospitality" />
          </div>
          <div style={{ width: 90 }}>
            <label style={label}>State</label>
        ...
        <label style={label}>Atlas region (optional — for regional bodies)</label>
        ...
        <label style={label}>Org / contact-page website</label>
        <input style={{ ...input, width: '100%', marginBottom: 10 }} value={form.website} onChange={(e) => set('website', e.target.value)} placeholder="https://www.tict.com.au/contact" />
        <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
          <div style={{ flex: 1 }}>
            <label style={label}>Contact email (optional — Discover can find one)</label>
            <input style={{ ...input, width: '100%' }} value={form.contact_email} onChange={(e) => set('contact_email', e.target.value)} placeholder="info@…" />
          </div>
          <div style={{ width: 130 }}>
            <label style={label}>LinkedIn (optional)</label>
            <input style={{ ...input, width: '100%' }} value={form.linkedin} onChange={(e) => set('linkedin', e.target.value)} placeholder="linkedin.com/…" />
          </div>
        </div>
        <button style={btnPrimary} onClick={addOrg} disabled={adding}>{adding ? 'Adding…' : 'Add contact'}</button>
        {addMsg && (
      ...
      <div style={{ ...card, padding: 18 }}>
        ...
        <textarea
          ...
          placeholder={'org_name,contact_name,role_title,org_type,focus,state,website,contact_email,kind\nTourism Industry Council Tasmania,,Secretariat,peak_body,"tourism, hospitality",TAS,https://www.tict.com.au,,org'}
        />
    ...
    <div style={{ background: '#fff', border: '1px solid var(--color-border, #e5e5e5)', borderRadius: 8, padding: '11px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontFamily: 'var(--font-body, system-ui)', fontSize: 14, fontWeight: 500, color: 'var(--color-ink, #2D2A26)' }}>
          {row.contact_name || row.org_name}
          <Chip color={row.kind === 'org' ? '#7a6f5f' : '#8a6520'}>{row.kind}</Chip>
        </div>
        <div style={{ fontFamily: 'var(--font-body, system-ui)', fontSize: 11, color: 'var(--color-muted, #888)', marginTop: 2 }}>
          {[row.contact_name ? row.org_name : row.role_title, (row.focus || []).join(' / '), row.state].filter(Boolean).join(' · ') || '—'}
...
function CampaignsPanel({ campaigns }) {
  if (!campaigns.length) {
    return <div style={{ textAlign: 'center', padding: '48px 0', fontFamily: 'var(--font-body, system-ui)', fontSize: 14, color: 'var(--color-muted, #888)' }}>No industry campaigns sent yet.</div>
  }
  return (
...
function AutopilotPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const d = await fetchJson('/api/admin/industry-outreach/settings')
      setData(d)
      setForm(d.settings)
    } catch (err) { setError(err.message) } finally { setLoading(false) }
  }, [])
  ...
  if (error && !form) {
    return (
      <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', padding: '12px 16px', borderRadius: 8, ...bodyFont, fontSize: 13, color: '#991B1B' }}>
        {error} {String(error).includes('relation') || String(error).includes('industry_outreach') ? '— run migration 257 first.' : ''}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 20 }}>
        <StatCard n={`${st.sent_today ?? 0} / ${form.daily_send_cap}`} label="introduced today (Melbourne)" />
        <StatCard n={`${st.followups_today ?? 0} / ${form.followup_daily_cap}`} label="follow-ups today" />
        <StatCard n={(st.sendable_pool ?? 0).toLocaleString()} label="ready to introduce" />
      ...
      {st.last_run && (
        <div style={{ ...bodyFont, fontSize: 12, color: 'var(--color-muted, #888)', marginBottom: 18 }}>
          ...
          {st.last_run.summary?.send?.sent != null ? ` · ${st.last_run.summary.send.sent} introduced` : ''}
      ...
      <div style={{ ...card, background: '#fff', padding: '6px 18px 2px', marginBottom: 16 }}>
        <SettingRow
          ...
          desc="Every weekday morning: scan the next unchecked org and contact pages for a contact email, and AI-write personal openers. No email is sent by this switch alone. All outreach email holds outside 9am–5pm Melbourne time."
        >
          <Toggle checked={form.enabled} onChange={(v) => setForm({ ...form, enabled: v })} />
        </SettingRow>
        <SettingRow
          title="Send first-touch introductions"
          desc="Let the autopilot send the daily Atlas-introduction batch (weekdays only). Every email carries one-click unsubscribe and all suppression rules apply."
        >
          <Toggle checked={form.send_enabled} onChange={(v) => setForm({ ...form, send_enabled: v })} disabled={!form.enabled} />
        </SettingRow>
        <SettingRow title="Daily send cap" desc="First-touch introductions per 24 hours. Keep this small — you court peak bodies, you do not blast them.">
        ...
        <SettingRow title="Follow-up after (days)" desc="How long to wait after the first introduction.">
          {num('followup_after_days', 2, 60)}
        </SettingRow>
        <SettingRow title="Follow-up daily cap" desc="Follow-ups per 24 hours.">
          {num('followup_daily_cap', 0, 60)}
        </SettingRow>
        <SettingRow title="Orgs scanned per run" desc="Email discovery throughput. Scanning is free — this just bounds the run time.">
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        ...
        <span style={{ ...bodyFont, fontSize: 12, color: 'var(--color-muted, #888)', marginLeft: 'auto', maxWidth: 400, lineHeight: 1.5 }}>
          The autopilot never emails suppressed, bounced, declined or already-contacted organisations, and holds all sending on weekends.
        </span>
      </div>
    </div>
  )
...
// ── Root ──────────────────────────────────────────────────────
export default function IndustryOutreachActions({
  ...
  return (
    <div>
      ...
      {tab === 'log' && (
        logRows.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '48px 0', fontFamily: 'var(--font-body, system-ui)', fontSize: 14, color: 'var(--color-muted, #888)' }}>No industry outreach yet.</div>
        ) : (
