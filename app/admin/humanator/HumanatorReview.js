...
function MilestoneBanner({ count }) {
  ...
  return (
    <div style={{
      ...
      animation: 'humanatorFadeInOut 3s ease-in-out forwards',
      letterSpacing: '0.02em',
    }}>
      <span style={{ marginRight: 8, color: 'var(--color-sage, #7A8B6F)' }}>&#10022;</span>
      {count.toLocaleString()} humanised
    </div>
  )
}
...
export default function HumanatorReview({ initialListing, initialStats }) {
  ...
  const handleAction = useCallback(async (action) => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/humanator', {
        method: 'PATCH',
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto', position: 'relative' }}>
      ...
      <style>{`
        @keyframes humanatorFadeInOut {
          0% { opacity: 0; transform: translateX(-50%) translateY(-8px); }
          12% { opacity: 1; transform: translateX(-50%) translateY(0); }
          80% { opacity: 1; transform: translateX(-50%) translateY(0); }
          100% { opacity: 0; transform: translateX(-50%) translateY(-8px); }
        }
        @keyframes humanatorPulse {
          0% { transform: scale(1); }
