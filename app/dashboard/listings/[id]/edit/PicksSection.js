 ...
 *
 * Renders as one collapsible Panel in the editor, locked when the listing isn't
 * paid and gated again until the claim on this listing is live.
 */
...
export default function PicksSection({ listingId, token, isPaid, listing }) {
  ...
  if (!isPaid) {
    return (
      <Panel id="picks" title={picksLabel} status="locked" summary="Included with Standard">
        <LockedNote>
          Recommend other places you rate across the Atlas network, and be recommended back. Included with Standard.
        </LockedNote>
      </Panel>
    )
  }

  // The claim gate: paid, but this listing isn't in the operator's own list yet.
  // Reads as "nothing here" in the header — the counter would be meaningless.
  const gated = !loading && !owns

  return (
    <Panel
      id="picks"
      title={picksLabel}
      status={picks.length ? 'done' : 'empty'}
      meta={gated ? null : `${picks.length} / ${maxPicks}`}
      summary={
        gated
          ? 'Available once your claim is active'
          : picks.length
            ? `${picks.length} place${picks.length === 1 ? '' : 's'} recommended`
            : 'No picks yet'
      }
    >
      <ConfirmDialog
