...
// Pick write-ups are capped at 50 words (the operator editor's limit).
export const MAX_NOTE_WORDS = 50

const SELECT_COLS = 'id, listing_id_a, listing_id_b, source, confidence, metadata, grandfathered, created_at'
...
function shapePick(r, map) {
  ...
  return {
    ...
    // Migration 263 — exempt from the paid-curator render gate (pre-dates the
    // gate, or admin editorial). Consumers render a non-grandfathered pick
    // only while its curator still holds a live standard claim.
...
// Enforces: required ids, no self-pick, a PAID curator, and the curator cap.
// The DB enforces no-duplicate. Returns { ok, pick } | { ok:false, error, code }.
//
// Picks are a Standard-tier perk, so the curator must hold a live standard
// claim (isListingPaid — active, or past_due dunning grace; a lapsed comp is
// already unpaid here). The check lives in this helper, not only in the route,
// so no future caller can write an ungated pick by accident: it defaults ON
// and has to be waived deliberately.
//
// `requirePaidCurator: false` is the admin waiver — network staff curating
// "picked by" from the listing editor, matching the `user.role === 'admin' ||
// isListingPaid(...)` bypass used everywhere else. An admin-created pick is
// written `grandfathered` so it actually renders (see migration 263);
// otherwise the waiver would insert a row the place page then hides.
export async function createPick(sb, {
  curatorId,
  pickedId,
  note = null,
  source = 'operator',
  createdBy = null,
  requirePaidCurator = true,
  grandfathered = false,
}) {
  ...
  if (requirePaidCurator && !(await isListingPaid(sb, curatorId))) {
    return {
      ok: false,
  ...
  if (note) {
    // The pick write-up is capped at 50 words (the operator editor's limit).
    const words = String(note).trim().split(/\s+/).filter(Boolean)
    if (words.length) metadata.note = words.slice(0, MAX_NOTE_WORDS).join(' ')
  }
