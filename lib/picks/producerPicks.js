...
// Producer's Picks is a paid perk: only a listing with an active *standard*
// claim may curate picks or surface them publicly. Curating is per-listing,
// so eligibility is checked per curator listing, not per user.
export const PICKS_REQUIRED_TIER = 'standard'
...
// Of the given listing ids, which are PAID — i.e. have an active standard
// claim in listing_claims? Returns a Set for O(1) membership tests. The
// canonical commercial state lives on listing_claims, never on listings, so
// this is the single source of truth for "may this listing use Producer's
// Picks". `sb` must be a master-portal client (where listing_claims lives).
