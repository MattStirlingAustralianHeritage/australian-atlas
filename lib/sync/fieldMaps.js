...
// Emit sub_type + sub_types from a single category/type value.
// sub_types (array) is canonical; the listings trigger
// trg_sync_sub_type_from_array derives the scalar sub_type from sub_types[1]
// (and NULLs it when sub_types becomes empty). The category → sub_type mapping
// drives the per-vertical category badges and facets that now read
// listings.sub_type LIVE from the portal (read-repair 2026-06-17), so this
// mapping is load-bearing — do not drop it from any listing map.
//
// When the source value is absent we return {} so the sync upsert OMITS both
// columns instead of writing nulls. That preserves a previously resolved
// sub_type rather than letting a momentarily-null source category wipe it
// (which would otherwise regress a backfilled value via the trigger above).
function subTypeFields(value) {
  return value ? { sub_type: value, sub_types: [value] } : {}
}
...
// Presence fields are only synced when the source table actually carries the
// column (the sync fetches select('*'), so a missing key means a missing
// column). Several vertical DBs never received migration 087 — for those the
// portal is where visitable/presence are curated (candidate approval +
// InlineListingEditor), and defaulting `?? true` here would clobber
// visitable=false makers back onto /map on every sync, resurrecting the
// phantom city-centroid cluster removed in 203_map_pins_visitable.sql.
function presenceFields(row) {
  const out = {}
  if ('visitable' in row) out.visitable = normalizeVisitable(row)
  if ('presence_type' in row) out.presence_type = normalizePresenceType(row)
  if ('market_appearances' in row) out.market_appearances = row.market_appearances || null
  return out
}

const HOURS_DAY_KEYS = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']
// Short-key → long-key map for sources that store {mon, tue, ...} (Fine
// Grounds roasters). Ordered so the expanded object keeps Mon–Sun order.
const HOURS_SHORT_TO_LONG = {
  mon: 'monday', tue: 'tuesday', wed: 'wednesday', thu: 'thursday',
  fri: 'friday', sat: 'saturday', sun: 'sunday',
}

// Normalize opening_hours from source into the canonical portal shape:
// a { monday: "...", ... } object with long day keys (migration 034).
// Three source shapes exist in the wild and all must round-trip:
//   1. text[] of "Monday: 9:00 AM – 5:00 PM" strings (SBA venues).
//   2. object with short day keys { mon: "7:00-16:00", ... } (FG roasters).
//   3. object with long day keys { monday: "...", ... } (Corner/Table/Found/
//      FG cafes) — already canonical, returned unchanged.
// Returns null when no meaningful hours data exists.
function normalizeHours(hours) {
  if (!hours) return null

  // Shape 1: array of "Day: value" strings. indexOf(':') splits on the FIRST
  // colon (the day/value separator) — the value keeps its own colons intact
  // ("11:00 AM – 10:00 PM"). "Monday: Closed" → { monday: "Closed" }.
  if (Array.isArray(hours)) {
    ...
    for (const entry of hours) {
      ...
      if (idx === -1) continue
      const day = entry.slice(0, idx).trim().toLowerCase()
      const value = entry.slice(idx + 1).trim()
      if (HOURS_DAY_KEYS.includes(day) && value) out[day] = value
    }
    return HOURS_DAY_KEYS.some(d => out[d]) ? out : null
  }

  if (typeof hours !== 'object') return null

  // Shape 3: already has long day keys with data — return unchanged so
  // existing consumers see byte-identical output to before this change.
  if (HOURS_DAY_KEYS.some(d => hours[d] && hours[d] !== '')) return hours

  // Shape 2: short day keys → expand to long keys, Mon–Sun order.
  if (Object.keys(HOURS_SHORT_TO_LONG).some(k => hours[k] && hours[k] !== '')) {
    const out = {}
    for (const [short, long] of Object.entries(HOURS_SHORT_TO_LONG)) {
      if (hours[short] && hours[short] !== '') out[long] = hours[short]
    }
    return HOURS_DAY_KEYS.some(d => out[d]) ? out : null
  }
...
export function mapWayListing(row) {
  ...
  return {
    ...
    is_market: false,
    ...subTypeFields(row.primary_type),
    status: row.status === 'published' ? 'active'
          : row.status === 'archived'  ? 'inactive'
          : 'inactive',                            // 'draft' → inactive on portal.
    // Way keeps its bespoke presence_type translation; visitable still only
    // syncs when the source table carries the column (see presenceFields).
    ...('visitable' in row ? { visitable: normalizeVisitable(row) } : {}),
