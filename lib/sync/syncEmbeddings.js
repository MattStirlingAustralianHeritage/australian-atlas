...
export async function generateEmbeddings({ maxListings = 1000, maxArticles = 200 } = {}) {
  ...
  if (listings && listings.length) {
    // Operator-authored published Q&A (migration 209) enriches each listing's
    // own embedding — one grouped fetch for the batch. Best-effort: a failure
    // (e.g. pre-migration DB) leaves qna empty and text is byte-identical to
    // before, so nothing regresses.
    const qnaByListing = new Map()
    try {
      const { data: qnaRows } = await sb
        .from('listing_qna')
        .select('listing_id, question, answer, published, position')
        .in('listing_id', listings.map((l) => l.id))
        .eq('published', true)
        .order('position', { ascending: true })
      for (const q of qnaRows || []) {
        if (!qnaByListing.has(q.listing_id)) qnaByListing.set(q.listing_id, [])
        qnaByListing.get(q.listing_id).push(q)
      }
    } catch (e) {
      console.warn('[embeddings] listing_qna fetch skipped:', e.message)
    }

    const items = listings.map((l) => ({
      l,
      text: buildListingText(l, regionName.get(l.region_override_id ?? l.region_computed_id), { qna: qnaByListing.get(l.id) }),
    }))
