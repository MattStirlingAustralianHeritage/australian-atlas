...
import { buildNameGuardIndex, applyNameGuard } from './nameGuard.js'
...
async function syncSourceRows({
  ...
  // Cross-entity name guard (incident 2026-07-23, Watts River Brewing —
  // see lib/sync/nameGuard.js). Refuse to apply a rename of an existing
  // master listing when the incoming name belongs to a DIFFERENT listing
  // in the network; keep the current name, sync the rest of the row, and
  // log loudly. buildNameGuardIndex returns null on a read failure and the
  // guard stands down for the run rather than blocking the sync.
  const nameGuardIndex = await buildNameGuardIndex(master)
  const blockedRenames = applyNameGuard(nameGuardIndex, vertical, items)
  for (const b of blockedRenames) {
    console.error(
      `[name-guard] BLOCKED rename ${vertical}:${b.source_id} ` +
      `"${b.kept_name}" → "${b.attempted_name}" — matches ` +
      b.conflicts.map((c) => `${c.vertical}:${c.source_id} "${c.name}" (${c.suburb || c.state || '?'})`).join(', ')
    )
  }
  ...
  return { synced, quarantined, quarantineReasons, errors, syncedSourceIds, blockedRenames }
}
...
export async function syncVertical(vertical) {
  ...
  const { synced, quarantined, quarantineReasons, errors, syncedSourceIds, blockedRenames } =
  ...
  return { vertical, synced, quarantined, quarantineReasons, deactivated, errors, blockedRenames }
}
...
export async function syncFineGrounds() {
  ...
  const accumulate = (res) => {
    ...
    allBlockedRenames.push(...(res.blockedRenames || []))
  }
  ...
  return {
    ...
    errors: totalErrors,
    blockedRenames: allBlockedRenames,
  }
}
