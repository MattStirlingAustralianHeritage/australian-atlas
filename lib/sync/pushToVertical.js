...
import { getSupabaseAdmin, getVerticalClient, VERTICAL_CONFIG, getClaimFieldConfig } from '@/lib/supabase/clients'
...
export async function triggerVerticalRevalidation(vertical, slug, category) {
...
export function mapToVerticalSchema(vertical, data) {
  ...
  // Ownership display state, only when the caller supplied it (candidate
  // pushes don't). Column name comes from VERTICAL_CLAIM_FIELD. Boolean
  // verticals sync symmetrically (grant → true, cancel → false); enum (way:
  // claim_status can be 'pending'/'paid') and uuid (found: owner_id) claim
  // models are richer than a boolean and must never be clobbered from here.
  if (data.is_claimed !== undefined) {
    const claimCfg = getClaimFieldConfig(vertical)
    if (claimCfg?.claimType === 'boolean') {
      base[claimCfg.claimField] = !!data.is_claimed
    }
  }
