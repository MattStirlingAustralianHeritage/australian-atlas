...
const VERTICAL_LISTING_PATH_PREFIX = {
  ...
  collection: '/venue',
  craft: '/venue',
  fine_grounds: '/roasters', // also /cafes for entity_type=cafe; revalidate prefix captures both via path matching on the vertical
  rest: '/stay',
  field: '/places',
  corner: '/shops',
  found: '/shops',
...
async function triggerVerticalRevalidation(vertical, slug) {
  const base = VERTICAL_BASE_URLS[vertical]
  const prefix = VERTICAL_LISTING_PATH_PREFIX[vertical]
...
export async function recordSyncAndRevalidate({
  ...
  if (verticalSuccess && slug) {
    revalidateStatus = await triggerVerticalRevalidation(vertical, slug)
  }
...
export const VERTICAL_CATEGORIES = {
  ...
  rest: ['boutique_hotel', 'guesthouse', 'bnb', 'farm_stay', 'glamping', 'cottage', 'self_contained', 'eco_resort'],
