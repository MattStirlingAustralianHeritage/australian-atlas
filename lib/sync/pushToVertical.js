...
export const VERTICAL_CATEGORIES = {
  ...
  table: ['restaurant', 'bakery', 'market', 'farm_gate', 'artisan_producer', 'specialty_retail', 'destination', 'cooking_school', 'providore', 'food_trail', 'cafe'],
}
...
export function mapToVerticalSchema(vertical, data) {
  ...
  switch (vertical) {
    ...
    case 'table': {
      // Table Atlas listings table uses website_url (not website) and may not have hero_image_url
      const { hero_image_url: _tableHero, ...tableBase } = base
      return {
        ...tableBase,
        suburb: data.suburb || data.region || null,
        lat: data.lat || null,
