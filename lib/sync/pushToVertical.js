...
export const VERTICAL_CATEGORIES = {
  ...
  collection: ['museum', 'gallery', 'heritage_site', 'cultural_centre', 'botanical_garden', 'sculpture_park', 'cinema', 'drive_in', 'live_music_venue', 'comedy_club', 'theatre'],
  craft: ['ceramics_clay', 'visual_art', 'jewellery_metalwork', 'textile_fibre', 'wood_furniture', 'glass', 'printmaking', 'leathermaker', 'shoemaker', 'clothing', 'fragrance_candles'],
  ...
  corner: ['bookshop', 'records', 'homewares', 'design_store', 'stationery', 'jewellery', 'toys', 'general', 'clothing', 'food_drink', 'plants', 'specialty_retail', 'other'],
  found: ['vintage_clothing', 'vintage_furniture', 'vintage_store', 'antiques', 'op_shop', 'books_ephemera', 'art_objects', 'market'],
  table: ['restaurant', 'bakery', 'market', 'farm_gate', 'artisan_producer', 'specialty_retail', 'destination', 'cooking_school', 'providore', 'food_trail', 'cafe', 'creamery', 'chocolatier', 'confectioner', 'tea_shop'],
...
export async function pushToVertical(vertical, data) {
  try {
    ...
    // Never let a null opening_hours overwrite operator-entered hours in the
    // vertical source DB. Hours are portal-authoritative for RENDER (read from
    // listings.opening_hours); this outbound copy must not clobber good source
    // data with null.
    if (safeRow.opening_hours == null) delete safeRow.opening_hours
...
export async function updateInVertical(vertical, sourceId, data) {
  try {
    ...
    const columns = await getTableColumns(client, config.url, table)
    const safeRow = stripUnknownColumns(verticalRow, columns, vertical, table)
    // See note above: a null opening_hours must not clobber operator hours.
    if (safeRow.opening_hours == null) delete safeRow.opening_hours
