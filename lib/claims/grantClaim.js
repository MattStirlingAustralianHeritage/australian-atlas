...
/**
 * Mint an address-proving link for `email` and send the branded invite. The
 * link verifies the address and lands on set-password via /auth/callback —
 * accounts sign in with email + password, never with a standing emailed link.
 * Best-effort: returns true if the email actually went out.
 *
 * The link TYPE is not cosmetic, because GoTrue accepts different types
 * depending on the account's state (measured against this project):
 *
 *                        existing + unconfirmed   existing + confirmed
 *     generateLink recovery        mints                  mints
 *     generateLink invite          mints          ERRORS "already registered"
 *
 * So 'invite' is only for an account being created; anything that already
 * exists must be sent 'recovery'. Getting this wrong is silent — the send
 * simply never happens — and it would hit the outage-confirmed cohort
 * exactly: those accounts ARE confirmed to GoTrue but are not proof to us,
 * so they land here needing a link that 'invite' would refuse to mint.
 *
 * `existingTokenHash` is passed on the provisioning path, where the caller has
 * already minted an invite token in the act of creating the user.
 */
async function mintAndSendAccessLink(sb, email, existingTokenHash = null) {
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.australianatlas.com.au'
  let tokenHash = existingTokenHash
  let linkType = 'invite'
  if (!tokenHash) {
    // Try recovery first: it is the one type that works whatever state the
    // account is in. 'invite' remains as a fallback for the narrow case of an
    // address GoTrue does not know at all.
    ...
    if (!tokenHash) {
      console.error('[grantClaim] could not mint an access link for', email)
      return false
    }
  }
  if (!process.env.RESEND_API_KEY) {
    console.error('[grantClaim] RESEND_API_KEY missing — access email NOT sent')
    return false
  }
  const url = `${siteUrl}/auth/callback?token_hash=${encodeURIComponent(tokenHash)}&type=${linkType}&next=/account`
  try {
    const { Resend } = await import('resend')
    const resend = new Resend(process.env.RESEND_API_KEY)
    const { from, replyTo, subject, html } = inviteEmail({ url })
    ...
    console.error('[grantClaim] access email send failed (claim continues):', err.message)
    return false
  }
}
...
async function resolveOrProvisionUser(sb, email) {
  ...
  if (prof) {
    ...
    if (!(await isEmailVerified(sb, prof.id))) {
      const linkSent = await mintAndSendAccessLink(sb, email)
  ...
  const linkSent = await mintAndSendAccessLink(sb, email, invited.properties?.hashed_token)
