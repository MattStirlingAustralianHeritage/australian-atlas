...
import { getListingRegion, LISTING_REGION_SELECT } from '../regions/index.js'
 ...
 *
 * Guard: checks share_kit_sent_at to prevent duplicate sends. Pass
 * options.force to bypass the guard (dashboard "Resend my share kit" —
 * app/api/dashboard/share-kit/route.js enforces its own 1/day limit).
 *
 ...
 * @param {{ force?: boolean }} [options] - force: re-send even if share_kit_sent_at is set
 * @returns {Promise<object>} { ok, skipped?, sent?, kit?, error? } — kit carries
 *   { subject, html, greeting, social_captions, newsletter_paragraph, listing_url }
 */
export async function sendShareKit(listingId, operatorEmail, operatorName, options = {}) {
  const force = options?.force === true

  if (!listingId || !operatorEmail) {
    console.log('[operator-amplification] Missing listingId or operatorEmail — skipping')
    return { ok: false, error: 'listingId and operatorEmail are required' }
  }
  ...
  try {
    ...
    const { data: listing, error: listingError } = await sb
      .from('listings')
      .select(`id, name, slug, vertical, region, suburb, state, description, hero_image_url, share_kit_sent_at, ${LISTING_REGION_SELECT}`)
      .eq('id', listingId)
      .single()
    ...
    // Guard: prevent duplicate sends (bypassed by an explicit force re-send)
    if (listing.share_kit_sent_at && !force) {
    ...
    if (claudeRes.ok) {
      ...
      try { const { reconcileAnthropicBudget } = await import('@/lib/ai/guardedAnthropic'); await reconcileAnthropicBudget(_resv, claudeData.usage) } catch {}
      ...
      <div style="max-width:600px;margin:0 auto;font-family:sans-serif">
        <div style="padding:24px 0">
          ...
          <div style="padding:16px 20px;border-radius:8px;background:#2d2a24;margin-bottom:24px">
            ...
            <p style="font-size:12px;color:#8a7a5a;margin:0 0 8px">${esc(verticalName)} Atlas · ${esc(getListingRegion(listing)?.name || listing.state || '')}</p>
    ...
    const subject = `Your ${verticalName} Atlas listing is live — here's how to share it`
    ...
    // ── 5. Mark as sent ──────────────────────────────────────
    // On a forced re-send this doubles as the durable "last sent" timestamp
    // the dashboard resend route rate-limits against (1/day per listing).
    ...
    await completeRun(runId, {
      status: 'success',
      summary: { listing_name: listing.name, operator_email: operatorEmail, forced: force || undefined },
    })
    ...
    return {
      ...
      kit: {
        ...
        newsletter_paragraph: newsletterParagraph,
