...
const VERTICAL_URLS = {
  ...
  rest: {
    ...
    label: 'Rest Atlas',
    tagline: 'Cabins, guesthouses, B&Bs, farm stays, and eco-lodges',
    brand_colour: '#5A8A9A',
    public: true,
  },
  field: {
  ...
  found: {
    ...
    label: 'Found Atlas',
    tagline: 'Vintage shops, antique dealers, op shops, and salvage yards',
    brand_colour: '#D4956A',
    public: true,
  },
  table: {
 ...
 * <TypographicCard>, the Discover deck, and the photo-less card treatments.
 * Text on these is always warm cream (#FAF8F4).
 *
 * These were a muddy near-monochrome earth family (every brown within a few
 * ΔE of the next), so a cluster grid read as one colour. They've been lifted
 * in saturation and pulled apart in hue so the ten verticals are legible as
 * distinct surfaces — wayfinding, not decoration: teal coffee, navy stays,
 * forest nature, olive tours, terracotta food, rust craft, plum culture.
 * Every ground keeps cream text above 8:1 contrast and sits ≥11 ΔE from its
 * nearest neighbour (verified). Culture stays plum (its own accent) rather
 * than a second green so it separates cleanly from Field in a grid.
 */
export const VERTICAL_CARD_TOKENS = {
  sba:          { bg: '#4E3613', text: '#FAF8F4', label: 'Small Batch Atlas' },  // amber / tobacco
  collection:   { bg: '#312B43', text: '#FAF8F4', label: 'Culture Atlas' },      // plum / indigo
  craft:        { bg: '#6A3A1C', text: '#FAF8F4', label: 'Craft Atlas' },        // rust
  fine_grounds: { bg: '#103F3C', text: '#FAF8F4', label: 'Fine Grounds Atlas' }, // deep teal
  rest:         { bg: '#143452', text: '#FAF8F4', label: 'Rest Atlas' },         // navy / ocean
  field:        { bg: '#18472D', text: '#FAF8F4', label: 'Field Atlas' },        // forest green
  corner:       { bg: '#3A514A', text: '#FAF8F4', label: 'Corner Atlas' },       // sage / pine
  found:        { bg: '#46392B', text: '#FAF8F4', label: 'Found Atlas' },        // aged taupe
  table:        { bg: '#5E2A1E', text: '#FAF8F4', label: 'Table Atlas' },        // terracotta / brick
  way:          { bg: '#3B3E1A', text: '#FAF8F4', label: 'Way Atlas' },          // olive
  portal:       { bg: '#0f0e0c', text: '#FAF8F4', label: 'Australian Atlas' },   // near-black master
}
