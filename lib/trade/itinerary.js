...
export async function buildStopRows(sb, itineraryId, stops) {
  ...
  for (const s of stops) {
    ...
    if (!l) continue
    // Day is one-based and clamped; time_hint is a short freeform string
    // ("10:00", "after lunch"). Both columns land in migration 204.
    const day = Math.min(60, Math.max(1, parseInt(s.day, 10) || 1))
    rows.push({
      itinerary_id: itineraryId,
      listing_id: l.id,
      position: position++,
      day,
      time_hint: s.time_hint ? String(s.time_hint).slice(0, 60) : null,
...
export async function hydrateStops(sb, stopRows) {
  ...
  return rows.map((s, i) => {
    ...
    return {
      ...
      sub_type: l && l.sub_type ? String(l.sub_type).replace(/_/g, ' ') : null,
