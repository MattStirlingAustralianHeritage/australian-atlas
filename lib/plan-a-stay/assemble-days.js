...
/* Nearest pool member to a centroid, preferring `preferFn` matches and
   avoiding already-used ids — but reusing the pool rather than returning
   nothing once every option has been spent (a meal anchor every day beats
   a gap). Returns null only when the pool is genuinely empty. */
function pickNearest(pool, centroid, usedIds, { preferFn, excludeIds } = {}) {
  if (!pool || pool.length === 0) return null
  const allowed = pool.filter(p => !(excludeIds && excludeIds.has(p.id)))
  if (allowed.length === 0) return null
  const unused = allowed.filter(p => !usedIds.has(p.id))
