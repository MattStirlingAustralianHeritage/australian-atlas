...
/* Table sub_types that are daytime PRODUCER/SHOP visits — the only Table
   stops that belong among activities. Anything else from Table (including
   an unknown or missing sub_type) reads as another sit-down meal, and one
   long lunch per day is enough. */
const TABLE_ACTIVITY_SUBTYPES = new Set([
  'farm_gate', 'market', 'creamery', 'providore', 'chocolatier',
  'confectioner', 'tea_shop', 'artisan_producer',
])

/* Field sub_types that are a real walk — the strenuous outdoor effort of a
   day. Common sense: one proper hike per day; lookouts, swimming holes and
   gardens are light stops that pair fine with it. */
const STRENUOUS_FIELD_SUBTYPES = new Set([
  'national_park', 'coastal_walk', 'bush_walk', 'gorge', 'waterfall',
  'nature_reserve',
])

export function isStrenuousListing(c) {
  return c.vertical === 'field' && STRENUOUS_FIELD_SUBTYPES.has(c.sub_type)
}
...
/* Alternates attached per day so the visitor can swap a stop for another
   real listing without a new retrieval round-trip. */
const ALTERNATE_ACTIVITIES_PER_DAY = 4
const ALTERNATE_MEALS_PER_DAY = 3
...
/* An "activity" is anything that is not a meal anchor or accommodation.
   ...
   are NOT meals — they stay as daytime activities. Every other Table
   sub_type (café, restaurant, unknown) is meal-slot material only: a day
   already has its coffee and its long lunch, and a second sit-down
   masquerading as an "activity" reads as nonsense. */
export function isActivityListing(c) {
  if (isRestListing(c)) return false
  if (c.vertical === 'fine_grounds') return false
  if (c.vertical === 'table' && !TABLE_ACTIVITY_SUBTYPES.has(c.sub_type)) {
    return false
  }
  return true
}
...
/* ─── Shape converters ───────────────────────────────────────────────────
   descMap values are either a plain description string (legacy) or an asset
   object { description, image_url } — both shapes are accepted so callers
   can migrate independently of this module. */
function assetFor(descMap, id) {
  const v = descMap.get(id)
  if (v == null) return { description: '', image_url: null }
  if (typeof v === 'string') return { description: v, image_url: null }
  return { description: v.description || '', image_url: v.image_url || null }
}

function toStop(c, descMap, mealSlot = null) {
  const asset = assetFor(descMap, c.id)
  return {
    listing_id: c.id,
    name: c.name,
    slug: c.slug || null,
    ...
    description_excerpt: excerptDescription(asset.description),
    image_url: asset.image_url,
    meal_slot: mealSlot,
  }
}

function toStay(c, descMap = new Map()) {
  const asset = assetFor(descMap, c.id)
  return {
    listing_id: c.id,
    name: c.name,
...
/* Activities: take up to `max`, deduped, with a per-sub_type variety cap
   and at most ONE strenuous walk per day — nobody hikes a gorge and a
   national park before dinner. Input is assumed pre-ranked (retrieve
   returns clusters score-ordered). */
...
export function buildDays({
  ...
  const days = clusters.map((cluster, i) => {
    ...
    const activityCandidates = candidates.filter(isActivityListing)
    const activities = pickActivities(activityCandidates, MAX_ACTIVITIES_PER_DAY)
    ...
    const accommodationOptions = nearestN(restPool, centroid, ACCOMMODATION_OPTIONS_PER_DAY)
      .map(c => toStay(c, descMap))
    ...
    const day = {
      ...
      // Cluster activity candidates NOT placed in this day — the seed of the
      // swap/add alternates attached below (clusters partition candidates, so
      // these can't collide with another day's stops).
      _leftoverActivities: activityCandidates.filter(c => !activities.some(a => a.id === c.id)),
    }
    return day
  })
  ...
  // ── Alternates — real swap options per day ─────────────────────────
  // Attached AFTER every day is assembled so meal alternates exclude every
  // coffee/lunch anchor already placed anywhere in the trip. A listing may
  // appear as an alternate on more than one day; the editing client retires
  // an alternate everywhere once it is placed.
  for (const d of days) {
    const freeCoffee = coffeePool.filter(p => !usedCoffee.has(p.id))
    const freeLunch = lunchPool.filter(p => !usedLunch.has(p.id) && !usedCoffee.has(p.id))
    d.alternates = {
      activities: d._leftoverActivities
        .slice(0, ALTERNATE_ACTIVITIES_PER_DAY)
        .map(c => toStop(c, descMap, null)),
      coffee: nearestN(freeCoffee, d.centroid, ALTERNATE_MEALS_PER_DAY)
        .map(c => toStop(c, descMap, 'coffee')),
      lunch: nearestN(freeLunch, d.centroid, ALTERNATE_MEALS_PER_DAY)
        .map(c => toStop(c, descMap, 'lunch')),
    }
    delete d._leftoverActivities
  }
