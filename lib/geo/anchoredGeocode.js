...
export async function anchoredGeocode(input, opts = {}) {
  ...
  // 1. Reliable anchor.
  const anchor = await resolveAnchor({ postcode, suburb, state }, token)
  ...
  // 4. Fall back to the locality centroid — right town / right region.
  if (anchor) {
    return finalize(anchor, anchor.kind === 'postcode' ? 'postcode' : 'suburb', 0, anchor.kind, derivedState)
  }
