...
import { extractStateFromPlaceName, deriveStateFromCoords } from './stateDerivation.js'
...
/**
 * Region/city-level geocode for a place with NO usable street address — resolves
 * a bare town / region / suburb NAME to its centroid, constrained to the listed
 * state. This is the correct pin for a listing that only knows its area
 * ("Noosa, QLD") rather than a building.
 *
 * Why not just reuse anchoredGeocode's suburb anchor? Because appending
 * ", {state}, Australia" to a short name makes Mapbox fuzzy-match the WHOLE
 * string — "Noosa, QLD, Australia" comes back as "Noona, New South Wales" — so
 * the pin lands in the wrong state. Here we query the BARE name (which returns
 * the real town at relevance 1.0) and pick the best result that actually sits in
 * the target state, validated two ways: the result's own place_name AND its
 * coordinates. Deliberately ignores any stored region FK, which can itself be
 * miscomputed (the bug that mis-pinned these listings in the first place).
 *
 * @param {{locality?: string, state?: string}} input
 * @returns {Promise<null | { lat, lng, precision:'locality', placeName, relevance }>}
 */
export async function localityCentroid({ locality, state } = {}, opts = {}) {
  const token = opts.token || process.env.NEXT_PUBLIC_MAPBOX_TOKEN || process.env.MAPBOX_ACCESS_TOKEN
  if (!locality || !token) return null
  // Strip a trailing ", STATE" (region text is often "Noosa, QLD") and any
  // parenthetical, then query the bare place name.
  const name = String(locality).split(',')[0].replace(/\(.*$/, '').trim()
  if (!name) return null
  const url = `${MAPBOX_BASE}/${encodeURIComponent(name)}.json?country=au&limit=5&types=place,locality,neighborhood&access_token=${token}`
  try {
    const res = await fetch(url, { signal: opts.signal ?? AbortSignal.timeout(8000) })
    if (!res.ok) return null
    const feats = (await res.json()).features || []
    const inState = (f) => {
      if (!state) return true
      const byName = extractStateFromPlaceName(f.place_name)
      if (byName) return byName === state
      // No parseable state in the name → fall back to the coordinate box.
      return deriveStateFromCoords(f.center?.[1], f.center?.[0]) === state
    }
    const f = feats.find(inState) // Mapbox returns results sorted by relevance
    if (!f) return null
    return { lat: f.center[1], lng: f.center[0], precision: 'locality', placeName: f.place_name || null, relevance: typeof f.relevance === 'number' ? f.relevance : 0 }
  } catch {
    return null
  }
}
