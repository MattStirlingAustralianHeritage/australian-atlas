...
import { reserveAnthropicBudget } from '@/lib/ai/guardedAnthropic'
...
export async function moderateImageBase64(base64, mediaType) {
  ...
  // Budget gate — fail CLOSED. Over the monthly cap → hold (never auto-clean).
  // A vision call is ~1.6k input tokens; max 256 out.
  const _resv = await reserveAnthropicBudget({ model: MODEL, inputTokens: 1600, maxOutputTokens: 256 })
  if (!_resv.ok) {
    return held('Moderation budget reached; held for review', 'budget', null)
  }
