...
// Supabase invite link that lands the operator on set-password. Miss that one
// link — it lands mid-service, it expires, it goes to spam — and until they
// set a password the only route back is a fresh emailed link, which nothing
// ever told the operator about. The 2026-07-29 audit
...
/**
 * @param {{ listingName?: string, claimantName?: string, signInUrl: string, listingUrl?: string,
 *           paid?: boolean, pendingVerification?: boolean }} p
 *
 * `pendingVerification` selects the post-265 wording. It is not cosmetic: for
 * those claimants the listing is genuinely NOT theirs yet — no ownership row
 * exists — so telling them "the listing is yours" would be false, and telling
 * them their dashboard is ready would send them to an empty one. The whole
 * point of the message is that one click is what completes the claim.
 *
 * @returns {{ from: string, replyTo: string, subject: string, html: string }}
 */
export function claimActivationEmail({
  listingName, claimantName, signInUrl, listingUrl,
  paid = false, pendingVerification = false,
} = {}) {
  const name = escapeHtml(listingName || 'your venue')
  const greetingName = claimantName ? escapeHtml(String(claimantName).split(' ')[0]) : null
  const url = signInUrl
  const subject = pendingVerification
    ? `One step left to finish your ${listingName || 'venue'} claim`
    : `Your dashboard for ${listingName || 'your venue'} is ready`
  ...
  const tierLine = paid
    ...
    : `<p style="${BODY_P}">From the dashboard you can keep your hours and contact details current and see how travellers are finding you.</p>`

  const openingLine = pendingVerification
    ? `<p style="${BODY_P}">We approved your claim on <strong style="color:#1C1A17; font-weight:500;">${name}</strong>, but it isn&rsquo;t finished: we still need to confirm this email address is yours. Until that happens the listing isn&rsquo;t assigned to anyone, so nothing is lost — it just isn&rsquo;t yours yet either.</p>
       <p style="${BODY_P}">Opening the link below is the whole of the remaining step. The one we sent when your claim was approved can only be used once, so here is a fresh one.</p>`
  ...
  // The "what you get" paragraph promises a populated dashboard, which is only
  // true once ownership exists. Withheld while the claim is still unconfirmed.
  const bodyHtml = `
    <p style="${BODY_P}">${greetingName ? `Hi ${greetingName},` : 'Hello,'}</p>
    ${openingLine}
    ${pendingVerification ? '' : tierLine}
    <p style="${BODY_P}">The link confirms your email address and asks you to choose a password — after that you sign in normally, any time. If the link has expired, use <strong style="color:#1C1A17; font-weight:500;">&ldquo;Forgot password?&rdquo;</strong> on the sign-in page and we&rsquo;ll email you a new one.</p>`
...
<body style="margin:0; padding:0; background:#faf8f5; -webkit-font-smoothing:antialiased;">
  <div style="display:none; max-height:0; overflow:hidden; opacity:0; mso-hide:all;">${pendingVerification ? `Confirm your email to finish claiming ${name}.` : `A fresh sign-in link for ${name} on the Australian Atlas.`}</div>
        ...
        <table role="presentation" width="520" cellpadding="0" cellspacing="0" border="0" style="width:520px; max-width:520px; background:#ffffff; border:1px solid #e7e3db; border-radius:14px;">
          <tr>
            <td class="card" align="center" style="padding:44px 48px 40px 48px;">
              ...
              <h1 style="margin:30px 0 0 0; font-family:'Playfair Display',Georgia,'Times New Roman',serif; font-size:27px; font-weight:400; color:#1C1A17; line-height:1.2; letter-spacing:-0.01em; text-align:left;">${pendingVerification ? 'One step left' : 'Your dashboard is ready'}</h1>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" align="center" style="margin:30px auto 0 auto;">
                <tr>
                  <td align="center" bgcolor="#1C1A17" style="border-radius:999px;">
                    <a href="${url}" style="display:inline-block; padding:15px 38px; font-family:'DM Sans',-apple-system,BlinkMacSystemFont,'Segoe UI','Helvetica Neue',Arial,sans-serif; font-size:15px; font-weight:500; color:#ffffff; text-decoration:none; border-radius:999px;">${pendingVerification ? 'Confirm my email' : 'Open my dashboard'}</a>
                  </td>
                </tr>
              </table>
