...
// Shared Atlas-branded chrome for the other auth emails (account access, recovery,
...
/**
 * Account access (admin-initiated recovery). The link verifies the address and
 * lands on the set-password screen — accounts sign in with email + password,
 * never with an emailed link.
 */
export function accountAccessEmail({ url }) {
  return {
    from: FROM, replyTo: REPLY_TO,
    subject: 'Access your Australian Atlas account',
    html: renderAuthEmail({
      title: 'Access your Australian Atlas account',
      preheader: 'Confirm your address and set your password. Valid for one hour.',
      headline: 'Access your account',
      bodyHtml: `<p style="${BODY_P}">Use the button below to confirm your email address and choose a password for your Australian Atlas account. The link is valid for one hour and can be used once — after that, sign in any time with your email and password.</p>`,
      ctaLabel: 'Set my password', ctaUrl: url,
      footerNote: 'If you didn&rsquo;t request this, you can safely ignore this email &mdash; nothing about your account will change.',
    }),
  }
}
