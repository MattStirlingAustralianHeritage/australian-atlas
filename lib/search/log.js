...
export function logSearchEvent(sb, f) {
  try {
    // Bot/template junk ("{search_term_string}", raw angle-bracket probes) is
    // not search telemetry — it poisons the zero-result report and trending.
    if (typeof f?.query_text === 'string' && /[{}<>]/.test(f.query_text)) return
