...
// Alphanumeric-only fold for name comparison — drops apostrophes, spaces and
// punctuation so "t'arts" and "T'Arts" compare on their letters alone.
function foldName(s) {
...
/**
 * Does a geocoded place NAME plausibly correspond to what the user typed?
 *
 * Mapbox `relevance` is not a sufficient gate: it strips punctuation and fuzzy-
 * matches aggressively, scoring "t'arts" → "Darts Creek" at 0.9 (both collapse
 * toward "arts"/"darts"). That hijacked a VENUE-name search (T'Arts Collective,
 * SA) into a proximity browse of Darts Creek, QLD. A genuinely typo'd town name
 * ("daylsford" → "Daylesford") stays near-identical to its match; a garbage
 * fuzzy leap does not. We accept only when the folded query is a clear prefix of
 * the place (a partial town name — "wilsons" → "Wilsons Promontory") or is
 * ≥ 0.62-similar to it by edit distance.
 */
export function geocodeLabelMatchesQuery(rawQuery, label) {
  const q = foldName(rawQuery)
  const p = foldName(label)
  if (!q || !p) return false
  if (q.length >= 4 && p.startsWith(q)) return true
  const sim = 1 - editDistance(q, p) / Math.max(q.length, p.length)
  return sim >= 0.62
}
...
export async function geocodePlace(rawQuery, { token, signal } = {}) {
  ...
  try {
    ...
    const label = (f.text || f.place_name || q).split(',')[0].trim()
    // Mapbox relevance rewards fuzzy leaps ("t'arts" → "Darts Creek" @ 0.9), so
    // require the returned name to actually match the query text — otherwise a
    // venue-name query gets hijacked into a wrong-town proximity browse.
    if (!geocodeLabelMatchesQuery(q, label)) return null
    return {
      lat: f.center[1],
      lng: f.center[0],
      label,
