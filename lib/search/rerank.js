...
export async function rerankSearchResults(sb, queryText, listings, opts = {}) {
  ...
  //    near-ties by the original fused position. The cross-encoder fixes gross
  //    errors (a name-coincidence match like "Stone & Wood" lands a whole tier
  //    below genuine wood-fired breweries), while equally-relevant candidates
  //    keep the first stage's fine signal — which already encodes is_claimed and
  //    quality_score, the business tiebreaks a pure neural sort would discard.
  ...
  const decorated = head.map((l, i) => ({ l, i, t: tierOf(l.id) }))
  decorated.sort((a, b) => (b.t - a.t) || (a.i - b.i))
  return { listings: decorated.map((d) => d.l).concat(tail), reranked: true }
}
