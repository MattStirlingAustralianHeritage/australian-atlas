...
export async function translateSearchQuery(q, lang) {
  ...
  try {
    ...
    // Hard timeout: this is the FIRST await in /api/search, so a hung Anthropic
    // call used to hang the entire CJK search path (vibeExpand has the same
    // race; translate didn't). Losing the race fails open to the raw query.
    const resp = await Promise.race([
      guardedAnthropicMessage(client, {
        model: MODEL,
        max_tokens: 120,
        system:
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('translate timeout after 2500ms')), 2500)),
    ])
