...
//
// Detection is WORD-BOUNDARY based, LONGEST-keyword-first, so the most specific
// signal wins ("boutique stay" beats "stay", "specialty coffee" beats "coffee")
// and a keyword can never fire from inside another word ('gin' in "ginger",
// 'stay' in "staysail" — real substring-era false leads). Keywords are the
// category words a searcher actually types for a vertical.
...
export const VERTICAL_INTENT_KEYWORDS = {
  ...
  craft: ['maker', 'makers', 'studio', 'studios', 'pottery', 'ceramics', 'woodwork', 'textiles', 'jewellery', 'chocolatier', 'chocolatiers', 'chocolate maker', 'chocolate makers'],
  // Fine Grounds — coffee
  ...
  corner: ['record store', 'bookshop', 'book shop', 'bookstore', 'homewares', 'design store', 'concept store', 'specialty shop', 'specialty retail', 'indie shop', 'nursery', 'plant nursery', 'garden centre', 'garden center', 'garden nursery', 'bottle shop', 'bottle-o', 'bottle store', 'wine merchant', 'wine shop', 'wine store', 'liquor store', 'craft beer shop'],
  ...
  table: ['food producer', 'farm gate', 'providore', 'restaurant', 'butcher', 'bakery', 'cheese', 'dining', 'pick your own', 'u-pick', 'fruit picking', 'strawberry picking', 'orchard'],
}

// Flatten to [keyword, regex, vertical] triples, longest keyword first (most
// specific match wins). Boundary-anchored via explicit [^a-z0-9] classes rather
// than \b: JS \b misfires on accented keywords ('café' at end of a query has no
// \b after the é), and lookbehind still has client-compat edges — this module
// runs in the browser too. Compiled once at module load.
const escapeRe = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
const INTENT_PAIRS = (() => {
  const pairs = []
  for (const [vertical, keywords] of Object.entries(VERTICAL_INTENT_KEYWORDS)) {
    for (const kw of keywords) {
      pairs.push([kw, new RegExp(`(?:^|[^a-z0-9])${escapeRe(kw)}(?:$|[^a-z0-9])`), vertical])
    }
  }
...
export function detectVerticalIntent(query) {
  ...
  for (const [kw, re, vertical] of INTENT_PAIRS) {
    if (re.test(lower)) return { vertical, keyword: kw }
  }
  return null
}
