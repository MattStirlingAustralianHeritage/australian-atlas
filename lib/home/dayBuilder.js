...
import { DAY_TRIP_SLOTS, slotEligible } from './daySlots.mjs'
import { openForSlot } from './openHours.mjs'

export { DAY_TRIP_SLOTS }
...
// Slot order and per-slot type eligibility live in ./daySlots.mjs —
// the editorial layer deciding what a worked day may contain.
...
function buildDayFromCandidates(candidates, seed, boostIds, coreOverride, nearKm = TRIP_MAX_KM_FROM_CENTRE) {
  ...
  // The day is built for TODAY, in each venue's own timezone: a venue
  // the hours say is closed today carries nothing, and one that is
  // closed over a slot's window can't carry that slot. Hours the row
  // doesn't have never exclude anything.
  const now = new Date()
  ...
  for (const { slot, labelKey, prefs } of DAY_TRIP_SLOTS) {
    for (let pi = 0; pi < prefs.length; pi++) {
      const [v, subTypes, opts] = prefs[pi]
      if (!opts?.shared && usedVerticals.has(v)) continue
      let candidatesForSlot = usable.filter(l =>
        l.vertical === v && (!subTypes || subTypes.includes(l.sub_type)) &&
        openForSlot(l, slot, now)
      )
      ...
      if (!opts?.shared) usedVerticals.add(v)
      // Pools stay disjoint — by vertical (usedVerticals), or for the
      // shared table prefs by disjoint sub_type lists — so no venue
      // can serve two slots. Cap for the route search below; the seeded
  ...
  // The hours payloads served their purpose at selection; the page
  // doesn't render them, so they stay out of the cached payload.
  return layers.map((layer, i) => {
    const { opening_hours, hours, ...stop } = picks[i]
    return { ...stop, slot: layer.slot, labelKey: layer.labelKey }
  })
}

// The public gates a stop must pass before it can carry a slot:
// a live /place page, real coordinates, real writing.
const DAY_CANDIDATE_SELECT = 'id, name, slug, vertical, sub_type, suburb, state, lat, lng, description, hero_image_url, is_claimed, visitable, address_on_request, trail_suitable, hours, opening_hours'
...
export async function assembleRegionDay(sb, regionName, seed, boostIds = null) {
  ...
  // Queried on the base table, not listings_with_region: the view's
  // `select l.*` was expanded when the view was created, so columns
  // added to listings since (hours, opening_hours) don't exist on it.
  // The region filter below restates the view's override-wins
  // resolution — override matches, or no override and the computed
  // region matches — NOT the old buggy plain OR that double-counted.
  let query = sb
    .from('listings')
    ...
    .not('lng', 'is', null)
    .not('name', 'ilike', '\\_%')
    // Fetch the region's whole candidate set (1000 is PostgREST's row
    // cap; the densest region holds ~660). The old 240 cap made dense
    // regions a lottery: which 240 of Melbourne's rows arrived was up
    // to heap order, so the same hour could build a day or fail to.
    // order('id') keeps the fetch deterministic even at the cap.
    .order('id')
    .limit(1000)
  if (resolved) {
    query = query.or(
      `region_override_id.eq.${resolved.id},and(region_override_id.is.null,region_computed_id.eq.${resolved.id})`
    )
  } else {
...
export async function assembleDayNearPoint(sb, point, seed) {
  ...
  const { data } = await sb
    ...
    // Same full-fetch rationale as assembleRegionDay: a dense metro
    // box can exceed a small cap, and an unordered partial fetch made
    // the build nondeterministic.
    .order('id')
    .limit(1000)
...
async function fetchClaimedIndex(sb) {
  try {
    const { data } = await sb
      .from('listings')
      .select('id, region, description, lat, lng, slug, visitable, address_on_request, trail_suitable, vertical, sub_type')
      .eq('status', 'active')
      .eq('is_claimed', true)
      .neq('vertical', 'way')
      .not('description', 'is', null)
