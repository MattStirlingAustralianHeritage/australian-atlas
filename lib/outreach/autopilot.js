...
export async function autopilotStatus(sb, settings) {
  ...
  // Work-pool counts exclude rows whose listing was deleted (listing_id goes
  // NULL, migration 258) — detached rows are contact history, not workable
  // candidates. Today's quota counts keep them: those sends really happened.
  const [sentToday, followupsToday, withEmailUnsent, needNote, followupDue, checked, lastRunRes] = await Promise.all([
    ...
    sb.from('operator_outreach').select('id', { count: 'exact', head: true }).not('contact_email', 'is', null).is('send_status', null).not('listing_id', 'is', null),
    sb.from('operator_outreach').select('id', { count: 'exact', head: true }).not('contact_email', 'is', null).is('send_status', null).is('personal_note', null).not('listing_id', 'is', null),
    sb.from('operator_outreach').select('id', { count: 'exact', head: true }).eq('send_status', 'sent').is('followup_sent_at', null).lte('sent_at', cutoff).not('status', 'in', '("claimed","replied","declined")').not('listing_id', 'is', null),
