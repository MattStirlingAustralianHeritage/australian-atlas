...
export const AUTOPILOT_DEFAULTS = {
  ...
  daily_send_cap: 40,       // first-touch emails per 24h
  ...
  followup_daily_cap: 40,   // follow-ups per 24h
}
...
export async function autopilotStatus(sb, settings) {
  ...
  const [sentToday, followupsToday, withEmailUnsent, needNote, followupDue, checked, lastRunRes] = await Promise.all([
    sb.from('operator_outreach').select('id', { count: 'exact', head: true }).gte('sent_at', dayAgo()),
    sb.from('operator_outreach').select('id', { count: 'exact', head: true }).gte('followup_sent_at', dayAgo()),
