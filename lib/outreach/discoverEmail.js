...
/**
 * Discover emails for many websites with bounded concurrency.
 * @param {Array<{ id: string, website: string }>} items
 * @param {number} concurrency
 * @returns {Promise<Array<{ id, website, email, candidates, source }>>}
 */
export async function discoverEmailsBatch(items, concurrency = 6) {
  const out = []
  let idx = 0
