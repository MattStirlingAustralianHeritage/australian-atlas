...
// Linear, length-bounded email matcher. The previous pattern nested unbounded
// quantifiers in the domain — `(?:[a-z0-9-]*[a-z0-9])?(?:\.…)+` — which
// catastrophically backtracks on adversarial page content (a long run of
// domain-ish chars that ultimately fails to match). That is pure synchronous CPU
// work: it blocks the event loop, so NO timer (the fetch AbortController, a
// per-site race, even the serverless maxDuration) can interrupt it — observed in
// prod as a single site pinning a worker for 47s and another for 243s, which
// ran the whole Discover function into a FUNCTION_INVOCATION_TIMEOUT. This form
// has a single bounded char class for the domain body (no nesting) plus explicit
...
const EMPTY_DISCOVERY = { email: null, candidates: [], source: null }

// Hard wall-clock cap on a single site. The per-fetch AbortController
// (FETCH_TIMEOUT_MS) usually bounds a site, but a pathological host — a hung DNS
// lookup, a slow-drip response body, a redirect loop — can evade it and stall a
// worker indefinitely. On a serverless function that stalls the WHOLE batch
// until the platform kills it (observed in prod: a 60s FUNCTION_INVOCATION_
// TIMEOUT 504 on a 10-site batch). Racing each site against a real timer
// guarantees the worker moves on, so the batch is deterministically bounded by
// ⌈items/concurrency⌉ × perSiteMs regardless of how any one host misbehaves.
function discoverWithCap(website, perSiteMs) {
  let timer
  const capped = new Promise((resolve) => {
    timer = setTimeout(() => resolve({ ...EMPTY_DISCOVERY, source: 'timeout' }), perSiteMs)
  })
  return Promise.race([
 ...
 * `perSiteMs` (so one bad host can't stall the batch), and when `deadlineMs` is
 * set workers stop picking up new sites once the budget is spent — the caller
 * gets partial results (and can see how many were processed via the shortened
 * length) instead of the request being killed mid-flight by the serverless
 * timeout, which would surface to the browser as a non-JSON 504.
 * @param {Array<{ id: string, website: string }>} items
 * @param {number} concurrency
 * @param {{ deadlineMs?: number, perSiteMs?: number }} [opts]
 * @returns {Promise<Array<{ id, website, email, candidates, source }>>}  shorter than items if the deadline hit
 */
export async function discoverEmailsBatch(items, concurrency = 6, { deadlineMs = 0, perSiteMs = 16000 } = {}) {
  const out = []
  let idx = 0
