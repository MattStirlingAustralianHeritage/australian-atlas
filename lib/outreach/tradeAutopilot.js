...
export const TRADE_AUTOPILOT_DEFAULTS = {
  ...
  daily_send_cap: 25,       // first-touch invites per 24h
  ...
  followup_daily_cap: 25,   // follow-ups per 24h
}
...
export async function tradeAutopilotStatus(sb, settings) {
  ...
  try {
    const [sentToday, followupsToday, withEmailUnsent, needNote, needDiscover, followupDue, lastRunRes] = await Promise.all([
      sb.from('trade_outreach').select('id', { count: 'exact', head: true }).gte('sent_at', dayAgo()),
      sb.from('trade_outreach').select('id', { count: 'exact', head: true }).gte('followup_sent_at', dayAgo()),
