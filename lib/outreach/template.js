...
export function buildMergeContext(listing, origin = SITE, personalNote = '', campaignId = '') {
  ...
  // auth links share the sending domain). Analytics reads utm_campaign.
 ...
 * @param {string} p.removeUrl       Fully-qualified "remove this listing" link (recipient + listing bound)
 * @param {string} p.personalNote    Optional AI/edited personal opener ({{personal_note}})
 * @returns {{ subject: string, html: string, text: string }}
 */
export function renderEmail({ subject, body, listing, origin = SITE, unsubscribeUrl, removeUrl = '', personalNote = '', campaignId = '' }) {
  ...
  const text = [
    ...
    removeUrl ? `Don't want ${ctx.name} listed? Remove the listing: ${removeUrl}` : '',
  ...
  const footer = `
    ...
    ${removeUrl
      ? `<p style="margin:0 0 6px;font-size:12px;line-height:1.6;color:#8a8378;">
      Don't want ${escapeHtml(ctx.name)} listed? <a href="${removeUrl}" style="color:#8a8378;text-decoration:underline;">Remove the listing</a>.
    </p>`
      : ''}
