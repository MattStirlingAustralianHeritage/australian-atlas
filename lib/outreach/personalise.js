 ...
 *
 * When `sb` is provided the call is metered through the monthly spend governor
 * (reserve-before, reconcile-after); a refused reservation returns the
 * sentinel so batch callers can stop instead of burning the queue.
 */
export const BUDGET_HIT = Symbol('anthropic-budget-hit')

export async function generatePersonalNote(listing, client, sb = null) {
  ...
  if (sb) {
    try {
      const { reserve, estimateAnthropicCost, estimateTokens } = await import('@/lib/budget/governor')
      const inputTokens = estimateTokens(SYSTEM_PROMPT) + estimateTokens(userMessage)
      reserved = estimateAnthropicCost(HAIKU_MODEL, inputTokens, MAX_TOKENS)
      const ok = await reserve(sb, 'anthropic', reserved)
      if (!ok) return BUDGET_HIT
    } catch (err) {
      // Fail-closed like every other governed call site.
      console.warn('[outreach/personalise] governor unavailable — skipping AI call:', err.message)
      return BUDGET_HIT
    }
  }
  try {
    ...
    if (sb && res.usage) {
      try {
        const { reconcile, estimateAnthropicCost } = await import('@/lib/budget/governor')
        const actual = estimateAnthropicCost(HAIKU_MODEL, res.usage.input_tokens || 0, res.usage.output_tokens || 0)
        await reconcile(sb, 'anthropic', actual - reserved)
      } catch { /* reservation stands — conservative */ }
    }
 ...
 * Pass `sb` to meter through the spend governor; on budget exhaustion the
 * remaining listings come back with '' and budgetHit=true.
 * @returns {Promise<{ notes: Array<{ id, personal_note }>, budgetHit: boolean }>}
 */
export async function generatePersonalNotesBatch(listings, concurrency = 4, sb = null) {
  const apiKey = resolveApiKey()
  if (!apiKey) return { notes: listings.map((l) => ({ id: l.id, personal_note: '' })), budgetHit: false }
  const client = new Anthropic({ apiKey })
  const out = []
  let idx = 0
  let budgetHit = false
  async function worker() {
    while (idx < listings.length && !budgetHit) {
      const i = idx++
      const l = listings[i]
      const note = await generatePersonalNote(l, client, sb)
      if (note === BUDGET_HIT) {
        budgetHit = true
        out[i] = { id: l.id, personal_note: '' }
        break
      }
  ...
  // Fill any slot skipped after a budget stop.
  for (let i = 0; i < listings.length; i++) {
    if (!out[i]) out[i] = { id: listings[i].id, personal_note: '' }
  }
  return { notes: out, budgetHit }
}
