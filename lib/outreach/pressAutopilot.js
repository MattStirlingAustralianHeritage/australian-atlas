...
export const PRESS_AUTOPILOT_DEFAULTS = {
  ...
  daily_send_cap: 15,       // first-touch emails per Melbourne day (small on purpose)
  ...
  followup_daily_cap: 15,   // follow-ups per Melbourne day
}
...
export async function pressAutopilotStatus(sb, settings) {
  ...
  try {
    const [sentToday, followupsToday, withEmailUnsent, needNote, needDiscover, followupDue, lastRunRes] = await Promise.all([
      sb.from('press_outreach').select('id', { count: 'exact', head: true }).gte('sent_at', quotaDayStart()),
      sb.from('press_outreach').select('id', { count: 'exact', head: true }).gte('followup_sent_at', quotaDayStart()),
