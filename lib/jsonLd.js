...
// Sub-type overrides for the vertical default @type. The collection vertical
// defaults to 'Museum', but several sanctioned collection primary types are
// factually not museums (cinemas, drive-ins, performing-arts venues). Emit the
// precise schema.org type for these so structured data doesn't mislabel a
// cinema as a Museum. Keyed by the listing's specific sub-type; anything not
// listed falls through to the vertical default.
const SUBTYPE_SCHEMA_TYPES = {
  cinema: 'MovieTheater',
  drive_in: 'MovieTheater',
  theatre: 'PerformingArtsTheater',
  live_music_venue: 'MusicVenue',
...
export function listingJsonLd(listing, extras) {
  // Prefer the sub-type-specific schema.org type (e.g. cinema → MovieTheater),
  // falling back to the vertical default. _subcategory is the meta-table source
  // of truth (institution_type); sub_type is the denormalised copy on listings.
  const listingSubType = listing._subcategory || listing.sub_type || null
  const schemaType = SUBTYPE_SCHEMA_TYPES[listingSubType] || VERTICAL_SCHEMA_TYPES[listing.vertical] || 'LocalBusiness'
