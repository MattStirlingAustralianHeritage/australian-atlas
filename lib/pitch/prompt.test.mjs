...
test('PHASE_2_PROMPT_VERSION matches current revision', () => {
  ...
  // v4 (2026-05-22): extended headline-grounding rule scope to include
  //   editorial_framing (closing the v3 "166-odd years" leak that surfaced
  //   in Morris v3 framing); added a thin-data positive example block
  //   (closing the v3 Perth Pottery "placeholder" regression); enumerated
  //   bail tokens explicitly in the prompt itself; orchestrator gained
  //   programmatic bail-token detection (pipeline.mjs detectBailToken).
  ...
  assert.equal(PHASE_2_PROMPT_VERSION, 'phase2-v4-2026-05-22')
})
...
test('system prompt contains every CRITICAL RULE verbatim from the spec', () => {
  ...
  const requiredFragments = [
    ...
    // v2/v3/v4 additions (2026-05-22) — see the dedicated tests below for
    // why each rule exists and what bug it closes.
    // The headline-grounding rule was reworded across v3 (paraphrase/
    // arithmetic) and v4 (scope extended to editorial_framing). The v4
    // opening sentence ("Every named entity… in your headline, angle, or
    // editorial_framing must trace to a verified_facts entry") is the
    // right substring to lock in here.
    'Every named entity, date, number, and factual claim in your headline, angle, or editorial_framing must trace to a verified_facts entry',
...
test('system prompt contains the headline grounding rule (v4)', () => {
  ...
  // example. v3 surfaced two new issues: (a) Perth Pottery regressed
  // to emit the literal string "placeholder" (data-thin candidate had
  // no template); (b) Morris's editorial_framing contained "166-odd
  // years" — same derived-numeric bug, relocated to a field the v3
  // rule didn't scope.
  //
  // Revision (v4, 2026-05-22): extends scope to editorial_framing
  // (closing leak (b)), enumerates bail tokens explicitly in the rule
  // (belt-and-braces with the new orchestrator-side detector), and
  // adds a thin-data positive example (closing leak (a) — see the
  // dedicated example test below).
  assert.ok(
    PHASE_2_SYSTEM_PROMPT.includes(
      'Every named entity, date, number, and factual claim in your headline, angle, or editorial_framing must trace to a verified_facts entry'
    ),
    'The v4 headline-grounding rule extends to editorial_framing and forbids literal bail tokens. Removing this scope or the bail-token list will cause regressions.'
  )
  // Lock in the paraphrase carve-out (preserved from v3).
  assert.ok(
    PHASE_2_SYSTEM_PROMPT.includes('PARAPHRASING is allowed and encouraged'),
    'The "PARAPHRASING is allowed and encouraged" clarification is missing from the headline grounding rule. Without it, the model treats the rule as a hard reject and emits empty placeholders (the v2 "x" bail).'
  )
  // Lock in the explicit arithmetic prohibition (preserved from v3).
  assert.ok(
  ...
  // v4 addition: the literal-bail-token list inside the rule. The v3
  // wording forbade "placeholder or empty string" but the model gamed
  // it by emitting the literal word "placeholder". The v4 list
  // enumerates the specific tokens, paired with the orchestrator-side
  // detectBailToken safety net.
  assert.ok(
    PHASE_2_SYSTEM_PROMPT.includes('the literal word'),
    'The "the literal word" enumeration of forbidden bail tokens is missing. Without it, the model can interpret "do not emit a placeholder" narrowly enough to satisfy it by emitting the literal token (the v3 Perth Pottery "placeholder" regression).'
  )
})

test('system prompt contains the grounded-headline positive examples (v4)', () => {
  // v3 had one positive example (data-rich, Morris-style). v3 surfaced
  // that the data-thin candidate (Perth Pottery) regressed because the
  // model had no template for thin-data headlines and bailed to
  // "placeholder". v4 adds a second example block anchoring thin-data
  // behaviour, so the model has a clear target on both ends of the
  // candidate spectrum.

  // Lock in the data-rich example (preserved from v3).
  assert.ok(
  ...
  // Lock in the NON-GROUNDED counterexample (preserved from v3).
  assert.ok(
    PHASE_2_SYSTEM_PROMPT.includes('NON-GROUNDED'),
    'The NON-GROUNDED counterexample is missing. The grounded/non-grounded contrast pair is what makes the positive example pedagogically useful.'
  )
  // v4 addition: thin-data positive example.
  assert.ok(
    PHASE_2_SYSTEM_PROMPT.includes('EXAMPLE OF A GROUNDED DATA-THIN HEADLINE'),
    'The thin-data positive example block header is missing.'
  )
  assert.ok(
    PHASE_2_SYSTEM_PROMPT.includes('Hands in the Clay on Beaufort Street'),
    'The thin-data positive example is load-bearing. Removing it caused the v3 Perth Pottery regression to "placeholder". The example shows the model that a thin-data headline can be modest and concrete without needing to be padded with derived material.'
  )
})
