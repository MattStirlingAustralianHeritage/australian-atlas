...
test('PHASE_2_PROMPT_VERSION matches current revision', () => {
  // Bumped to v2 on 2026-05-22 when the headline-grounding and atomic-claims
  // rules were added (see the two dedicated rule-presence tests below). Per
  // spec discipline, any prompt revision invalidates prior calibration-gate
  // runs — keep this assertion exact so a silent revision can't slip past.
  assert.equal(typeof PHASE_2_PROMPT_VERSION, 'string')
  assert.equal(PHASE_2_PROMPT_VERSION, 'phase2-v2-2026-05-22')
})
...
test('system prompt contains every CRITICAL RULE verbatim from the spec', () => {
  ...
  const requiredFragments = [
    ...
    // v2 additions (2026-05-22) — see the two dedicated tests below for
    // why each rule exists and what bug it closes.
    'Every named entity, date, number, and factual claim that appears in your headline or angle MUST also appear as a verified_facts entry',
...
test('system prompt contains the headline grounding rule (v2)', () => {
  // Added 2026-05-22 after the Morris Whisky dry-run surfaced "166 Years"
  // in the headline — a derived numeric (2026 - 1859) that did NOT appear
  // in any verified_facts entry. Fact-check validates the verified_facts
  // array but not headline/angle prose, so anything the model puts in the
  // headline that isn't also in verified_facts bypasses the architectural
  // anti-hallucination guarantee. The rule below closes that gap.
  assert.ok(
  ...
  // Also lock in the specific "derived calculations" guidance — the model
  // needs to know that arithmetic on grounded values is itself a factual
  // claim, not a free editorial inference.
  assert.ok(
    PHASE_2_SYSTEM_PROMPT.includes('Derived calculations'),
    'The "Derived calculations" clarification is missing from the headline grounding rule. Without it, the model treats arithmetic on founding dates as creative framing rather than factual claims requiring grounding.'
  )
})
