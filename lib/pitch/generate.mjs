...
// the LLM call.
//
// Model: claude-opus-4-7 with adaptive thinking + effort=high. The system
// prompt is the architectural anti-hallucination contract; the tool schema is
// the structured-output contract. Both live in prompt.mjs and are versioned
// via PHASE_2_PROMPT_VERSION.
//
// Streaming + .finalMessage() is used to dodge HTTP idle-timeout risks on
// adaptive-thinking calls and to handle tool-use blocks cleanly (the helper
// accumulates the full Message including the forced tool_use block).
//
...
export { buildUserMessage, renderListingRecord, parseResponse }
