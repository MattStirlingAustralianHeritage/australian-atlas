...
export async function generatePitch(anchorListing, opts = {}) {
  ...
  const userMessage = buildUserMessage(anchorListing, slotType, opts.feedback)
 ...
 *
 * `feedback` (optional) carries the reason a prior attempt was rejected — by
 * the fact-check pass and/or the prose verification gate — so a recomposition
 * can avoid the same mistake. See renderFeedback.
 */
function buildUserMessage(listing, slotType, feedback) {
  const slotTypeBlock =
...
Generate the pitch by calling the \`submit_pitch\` tool. Cite each verified fact with the exact column name from the record above. If the data is too thin to ground a complete brief, call \`report_insufficient_data\` with a specific reason instead — do not fabricate to fill gaps.${renderFeedback(feedback)}`
}

/**
 * Render a recomposition-feedback block appended to the user turn. Empty string
 * when there is no feedback (the common first-attempt case). Fact-check
 * failures list the verified_facts that did not trace to the cited field;
 * verification-gate flags list the prose claims that were not literally
 * supported. Either way the instruction is: do not repeat these; re-ground or
...
function renderFeedback(feedback) {
  ...
  if (Array.isArray(feedback.failedClaims) && feedback.failedClaims.length) {
    const lines = feedback.failedClaims
      .map(c => `  - "${c.fact?.claim ?? c.claim ?? '?'}" (cited field=${c.fact?.field ?? c.field ?? '?'}) — ${c.reason ?? 'did not match the source value'}`)
      .join('\n')
    parts.push(
      `A PRIOR ATTEMPT WAS REJECTED BY FACT-CHECK. These declared facts did not trace to the named field's value in the record:\n${lines}\nDo not repeat them. Cite only values that appear verbatim in the record.`
    )
  }
  ...
  if (Array.isArray(feedback.verifyFlags) && feedback.verifyFlags.length) {
    const lines = feedback.verifyFlags
      ...
      .join('\n')
    parts.push(
      `A PRIOR ATTEMPT WAS REJECTED BY PROSE VERIFICATION. These claims in the headline, angle, or editorial_framing were not literally supported by the data — derived numbers (e.g. an age computed from a founding year), inferences beyond the record, recombinations of separate facts, and absent assertions are ALL forbidden:\n${lines}\nRewrite to avoid every one of them. If an angle cannot be grounded without these claims, choose a different angle that the record fully supports, or call report_insufficient_data.`
    )
  }
