...
// the LLM call.
//
// Model: claude-opus-4-7 with effort=high. Adaptive thinking is NOT used —
// it returns a 400 when combined with forced tool_choice ("Thinking may not
// be enabled when tool_choice forces tool use"). Forced tool_choice is the
// structural guarantee that every response is structured output; that's
// load-bearing and stays. The anti-hallucination guarantee lives in
// fact-check, not in deliberation depth. The system prompt is the
// architectural anti-hallucination contract; the tool schema is the
...
// and to keep headroom under HTTP idle timeouts at effort=high.
//
// Prompt caching: the system prompt is marked cache_control: ephemeral. The
// effective prefix is shorter than the 4096-token Opus 4.7 cache minimum, so
// caching is unlikely to engage in practice during Phase 2 — the marker is
// retained as a no-op breakpoint so future prompt growth caches automatically.
// ─────────────────────────────────────────────────────────────────────────────
...
export const PHASE_2_MODEL = 'claude-opus-4-7'
...
const LLM_REQUEST_DEFAULTS = Object.freeze({
  ...
  // Do NOT add `thinking` here. Opus 4.7 adaptive thinking + forced
  // tool_choice 400s with "Thinking may not be enabled when tool_choice
  // forces tool use." Effort: 'high' retained for capability without the
  // conflict. Anti-hallucination guarantee lives in fact-check, not in
  // deliberation depth.
