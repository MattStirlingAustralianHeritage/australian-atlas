 ...
 */
export const PHASE_2_PROMPT_VERSION = 'phase2-v4-2026-05-22'
...
- Every named entity, date, number, and factual claim in your headline, angle, or editorial_framing must trace to a verified_facts entry. PARAPHRASING is allowed and encouraged — if verified_facts contains a fact citing the description field with value "six generations of fortified winemaking," you may write "generations-old winemaking heritage" in any of these fields as a paraphrase. ARITHMETIC and DERIVATION are not allowed — you may not compute new values (age in years from a founding date, totals, percentages, comparisons across facts) and include them in headline, angle, or editorial_framing without those derived values appearing as their own verified_facts entries. When you cannot ground a claim through verified_facts and paraphrase won't reach the meaning you want, choose a different angle to lead with; do not emit a placeholder, an empty string, the literal word "placeholder", "x", "tbd", "todo", or any similar bail token. The pitch must always have a real headline, a real angle, and real editorial framing — if you cannot write any of these with the grounded material available, return the structured "insufficient data" response instead.

EXAMPLE OF A GROUNDED DATA-RICH HEADLINE: For a distillery with founded_year=1859 and a description citing tokay-cask whisky finishing, a grounded headline is "Morris of Rutherglen finishes single malt in tokay casks" — every named element (Morris, Rutherglen, single malt, tokay casks) traces to verified_facts. A NON-GROUNDED variant is "Morris Whisky's 166 Years of Heritage" — "166 years" is a derived numeric not in verified_facts. The first is acceptable; the second is not.

EXAMPLE OF A GROUNDED DATA-THIN HEADLINE: For a new-producer ceramicist studio with no founded_year, no awards, and a description citing hand-building classes at a Beaufort Street studio, a grounded headline is "Hands in the Clay on Beaufort Street" — every named element (Beaufort Street, clay/hand-building) traces to verified_facts. Note that this headline contains no numbers, dates, or derived claims, because the source data does not support them. This is correct — a thin-data headline should be modest and concrete, not contrived. Do not pad thin-data headlines with derived or imagined material to make them sound richer.

- Each verified_facts entry must contain exactly ONE atomic claim citing ONE database field. Do not aggregate multiple sub-claims into a single entry. If you have five claims that all cite the description field, emit five separate verified_facts entries with five separate claim strings, not one entry whose claim string strings them together. Atomic claims are the only way fact-check can validate each one independently.
