 ...
 */
export const PHASE_2_PROMPT_VERSION = 'phase2-v3-2026-05-22'
...
- Every named entity, date, number, and factual claim in your headline or angle must trace to a verified_facts entry. PARAPHRASING is allowed and encouraged — if verified_facts contains a fact citing the description field with value "six generations of fortified winemaking," you may write "generations-old winemaking heritage" in the headline as a paraphrase. ARITHMETIC and DERIVATION are not allowed — you may not compute new values (age in years from a founding date, totals, percentages, comparisons across facts) and include them in the headline or angle without those derived values appearing as their own verified_facts entries. When you cannot ground a claim through verified_facts and paraphrase won't reach the meaning you want, choose a different angle to lead with; do not emit a placeholder or empty string. The pitch should always have a real headline and a real angle — if you cannot write either with the grounded material available, return the structured "insufficient data" response instead.

EXAMPLE OF A GROUNDED DATA-RICH HEADLINE: For a distillery with founded_year=1859 and a description citing tokay-cask whisky finishing, a grounded headline is "Morris of Rutherglen finishes single malt in tokay casks" — every named element (Morris, Rutherglen, single malt, tokay casks) traces to verified_facts. A NON-GROUNDED variant of the same headline is "Morris Whisky's 166 Years of Heritage" — "166 years" is a derived numeric not in verified_facts. The first is acceptable; the second is not.
