...
export async function runPipeline(candidate, opts) {
  ...
  // Bail-token detected — no log (failure_mode enum has no 'bail_token_detected'
  // value; extending the enum is a deferred migration), no retry (regenerating
  // doesn't fix a model decision to bail; the prompt change is the real fix).
