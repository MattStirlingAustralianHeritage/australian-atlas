...
export async function runPipeline(candidate, opts) {
  ...
  // failure_mode = 'bail_token_detected' (enum value added via migration 131,
  // 2026-05-22). No retry — regenerating doesn't fix a model decision to bail;
  // the prompt change is the real fix. The prompt forbids these tokens
  // explicitly; this check + log is the programmatic safety net + audit
  // trail for any new bail variants that slip past the prompt.
  if (attempt1.kind === 'bail_token_detected') {
    if (!dryRun) {
      await logFailure(supabase, {
        listingId: listing.id,
        slotId: slot?.id ?? null,
    ...
    return {
      ...
      logged: !dryRun,
      note:
        `LLM emitted a bail token in ${attempt1.bail.field} ` +
        `(value=${JSON.stringify(attempt1.bail.value)}, reason=${attempt1.bail.reason}). ` +
  ...
  // Same bail check as attempt 1 — logs to pitch_generation_failures.
  if (attempt2.kind === 'bail_token_detected') {
    if (!dryRun) {
      await logFailure(supabase, {
        listingId: listing.id,
        slotId: slot?.id ?? null,
    ...
    return {
      ...
      logged: !dryRun,
      note:
        `LLM emitted a bail token on attempt 2 in ${attempt2.bail.field} ` +
        `(value=${JSON.stringify(attempt2.bail.value)}, reason=${attempt2.bail.reason}).`,
    }
  }
