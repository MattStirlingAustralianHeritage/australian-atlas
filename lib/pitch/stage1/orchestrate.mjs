    ...
    const signalRows = valid.venue_signals
      .map(signal => {
        ...
        return {
          ...
          source_id: sourceId,
          // signal_data arrives from the LLM as a JSON-encoded string
          // (strict-mode workaround — see prompt.mjs). Parse it back into
          // an object for the jsonb column. Defensive fallback to {} on
          // malformed JSON so a single bad payload doesn't poison the
          // whole insert batch.
          signal_data: parseSignalData(signal.signal_data),
        }
      })
      .filter(Boolean)
...
/**
 * Parse the LLM's signal_data string into an object. The Stage 1 schema
 * makes signal_data a JSON-encoded string (strict-mode workaround); the
 * pitch_signals.signal_data DB column is jsonb. Defensive fallback to {}
 * on malformed JSON, null, or already-parsed objects.
 *
 * Returns an object suitable for direct INSERT into a jsonb column.
 */
export function parseSignalData(raw) {
  if (raw == null) return {}
  // Forgiving of already-parsed objects (the orchestrator's unit tests
  // pre-parse for fixture clarity, and a future caller might too).
  ...
  try {
    ...
    // Defensive: rather than failing the whole insert batch, fall back to
    // {} and let the audit trail capture the source_excerpt. The
    // structured data is editorially useful but not load-bearing for
    // grounding (the excerpt is the proof).
    return {}
  }
}
