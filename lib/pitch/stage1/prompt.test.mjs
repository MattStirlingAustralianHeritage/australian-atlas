...
test('PHASE_3_STAGE_1_PROMPT_VERSION matches current revision', () => {
  ...
  assert.equal(PHASE_3_STAGE_1_PROMPT_VERSION, 'phase3-stage1-v1-2026-05-22')
})
...
test('signal_data is a free-shape object (no required keys; type: object)', () => {
  // signal_data is jsonb on the DB side with no constraints; the LLM picks
  // appropriate fields per signal_type. Tool schema must declare type:
  // 'object' (Anthropic strict mode rejects missing type) but should not pin
  // required keys.
  const sig = SUBMIT_EXTRACTION_TOOL.input_schema.properties.venue_signals.items
  assert.equal(sig.properties.signal_data.type, 'object')
  assert.equal(sig.properties.signal_data.required, undefined)
})
