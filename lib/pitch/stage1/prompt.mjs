 ...
 */
export const PHASE_3_STAGE_1_PROMPT_VERSION = 'phase3-stage1-v2-2026-05-22'
...
- Venue signals capture non-character facts: founder pivots, recent openings, awards mentioned on-site, methodology novelty, cross-references to other venues. Each signal must trace to a source_excerpt the same way attributes do. The signal_data field must be a JSON-encoded string (e.g. \`"{\\"name\\":\\"James Halliday Award\\",\\"year\\":\\"2024\\"}"\`), not a raw object — the orchestrator parses it back. Use \`"{}"\` if there is nothing structured to add.
      ...
      venue_signals: {
        ...
        items: {
          ...
          properties: {
            ...
            signal_data: {
              // String-typed at the schema layer so the shape can stay
              // open per signal_type — Anthropic strict mode requires
              // `additionalProperties: false` on every nested object,
              // which is incompatible with the open-shape requirement
              // for signal_data. Same workaround Phase 2 uses for
              // verified_facts.value. The orchestrator JSON.parse's
              // this back into an object before inserting into the
              // jsonb pitch_signals.signal_data column.
              type: 'string',
              description:
                'A JSON-encoded object with type-specific fields. Use your judgement to capture editorially-useful structure. For award: {"name": "...", "year": "...", "awarding_body": "..."} when in source. For recently_opened: {"opened_date_text": "..."} if a date is given. For press_coverage: {"publication": "...", "headline": "..."} if those appear. Keep it minimal — what a researcher would want to look up. MUST be valid JSON (the orchestrator parses it). If there is nothing structured to add beyond the source_excerpt, return "{}".',
            },
          },
