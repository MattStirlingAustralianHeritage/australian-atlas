...
test('exports generatePitch and PHASE_2_MODEL', () => {
  ...
  assert.equal(PHASE_2_MODEL, 'claude-sonnet-4-6')
})

test('LLM_REQUEST_DEFAULTS pins the request shape', () => {
  // Positive assertions: the defaults we want to keep stable.
  assert.equal(LLM_REQUEST_DEFAULTS.model, 'claude-sonnet-4-6')
  assert.ok(LLM_REQUEST_DEFAULTS.max_tokens >= 8000)
})

test('LLM_REQUEST_DEFAULTS must NOT include `output_config`', () => {
  // output_config.effort was an Opus-only knob; dropped with the move to
  // Sonnet 4.6. Re-adding it here would send an unsupported field to the API.
  assert.equal(
    LLM_REQUEST_DEFAULTS.output_config,
    undefined,
    'LLM_REQUEST_DEFAULTS.output_config must be absent — effort was Opus-only and is dropped for Sonnet 4.6.'
  )
})
...
test('LLM_REQUEST_DEFAULTS must NOT include `thinking`', () => {
  ...
  //  7 May 2026.) This holds on every model, Sonnet 4.6 included.
  // Phase 2 uses forced tool_choice for the structured-output guarantee,
  // which makes adaptive thinking impossible. The anti-hallucination
  // guarantee lives in fact-check + the prose verification gate, not in
  // deliberation depth. Re-adding a `thinking` field here will break every
  // real LLM call.
  assert.equal(
    LLM_REQUEST_DEFAULTS.thinking,
    undefined,
    'LLM_REQUEST_DEFAULTS.thinking must be absent — adaptive thinking is incompatible with forced tool_choice (which Phase 2 requires for the structured-output guarantee). Re-adding it will break every real LLM call.'
  )
})
...
test('parseResponse extracts a submit_pitch tool input', () => {
  ...
  assert.ok(r.generated_by.startsWith('claude-sonnet-4-6'))
