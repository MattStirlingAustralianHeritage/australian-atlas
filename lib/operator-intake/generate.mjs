...
import { SYSTEM_PROMPT, STORY_QUESTIONS, bannedPhraseCheck } from './voice.mjs'
...
/**
 * Render the guided-interview answers (operator_stories.answers, keyed by
 * STORY_QUESTIONS ids) as a labelled Q/A block, or null when nothing is
 * answered. Shared by the first-write and rewrite prompts.
 */
export function renderStoryAnswers(answers) {
  if (!answers || typeof answers !== 'object' || Array.isArray(answers)) return null
  const lines = []
  for (const { key, q } of STORY_QUESTIONS) {
    if (str(answers[key])) lines.push(`Q: ${q}\nA: ${answers[key].trim()}`)
  }
...
export function buildUserMessage({ facts, listing = null }) {
  ...
  if (str(F.coverage_request)) {
    lines.push(
      '',
      `What the operator most wants this description to cover or add, in their own words — treat it as source material and work it in where it fits the structure, but invent nothing beyond what they wrote: ${F.coverage_request.trim()}`,
    )
  }

  const storyBlock = renderStoryAnswers(F.story_answers)
  if (storyBlock) {
    lines.push(
      '',
      'Interview answers in the operator’s own words. The same rule binds: source material only — draw details from these where they fit the structure, invent nothing beyond them.',
      storyBlock,
    )
  }
