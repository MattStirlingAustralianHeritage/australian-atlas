...
// ── Authorship ───────────────────────────────────────────────────────────────
// Every published description carries one of two bylines. 'atlas' is the
// guided-interview path (facts → Claude in the house voice → gates → review);
// 'owner' is the operator's own words, submitted verbatim through the same
// admin review gate. Stored as source_facts.authorship on the draft row —
// legacy drafts predate the marker, but every one of them was generated, so a
// recorded model implies 'atlas'.
export const AUTHORSHIP_LABELS = {
  owner: 'Written by the owner',
  atlas: 'Written by Atlas',
}

export function draftAuthorship(draft) {
  const marked = draft?.source_facts?.authorship
  if (marked === 'owner' || marked === 'atlas') return marked
  return draft?.model ? 'atlas' : 'owner'
}

// Bounds for owner-written copy. The floor keeps "great pub" out of the review
// queue; the ceiling matches the render layer (the place page shows the text in
// full, so a submitted essay would become the page).
export const OWNER_TEXT_LIMITS = { minChars: 40, maxChars: 1500 }
...
// The guided interview, absorbed from the retired "Your Story" feature
// (migration 210). Answers live in operator_stories.answers (jsonb keyed by
// these ids) and are optional grounding for description generation — every
// draft still passes the banned-phrase and source-binding gates and the admin
...
export const INTAKE_FIELDS = [
  {
    ...
    label: 'What you’d like covered',
    type: 'paragraph',
    help: 'In your own words: what you want written about, mentioned, or added — a new space, a change, an award, the things that matter to you. We’ll work it into the Atlas voice. We won’t invent anything you haven’t told us.',
    required: false,
  },
  {
