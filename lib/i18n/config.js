...
// English copy spells small counts out ("ten directories"); ko/zh copy pairs a
// numeral with a counter word (10개 디렉토리 / 10 个目录), so every non-default
// locale must receive the numeral, never the English word.
export function localizedCountWord(locale, englishWord, count) {
  return locale === defaultLocale ? englishWord : count
}
