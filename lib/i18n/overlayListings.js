...
export async function overlayListingTranslations(listings, locale, sb = null) {
  ...
  try {
    ...
    return listings.map((l) => {
      ...
      // Region localizes for every card, even ones without a name/desc row yet.
      if (!tr) return l.region ? { ...l, region: localizeRegionName(l.region, locale) } : l
      ...
      return {
        ...
        // Localize the region label shown on cards (Korean for known regions,
        // English fallback for anything else — incl. messy free-text values).
        region: l.region ? localizeRegionName(l.region, locale) : l.region,
        description:
