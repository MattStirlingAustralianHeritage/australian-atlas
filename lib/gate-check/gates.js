...
export async function checkGate1Web(listing, opts = {}) {
  ...
  // Unrelated-business check: does the page mention the listing name at all?
  // Very conservative (<34% word overlap) so redirects/rebrands don't false-fire.
  const words = (listing.name || '').toLowerCase().split(/\s+/).filter(w => w.length > 2)
  if (words.length) {
    let matched = 0
    for (const w of words) if (lower.includes(w)) matched++
    const ratio = matched / words.length
    if (ratio < 0.34) {
      return { failure: { gate: 'gate1_web', code: 'name_mismatch', severity: 1,
        reason: `Website never mentions "${listing.name}" (${Math.round(ratio * 100)}% name match) — the URL may point at an unrelated site.` }, text, lastModified, http_status }
    }
  }
