...
// ═══ GATE 6 — Hallucination (fabricated / mis-identified venue) ══════════════
//
// Not a computed gate — there is no reliable deterministic test for "this venue
// was invented by an automated populator". It is raised out-of-band by a human
// or a verification agent that has web-checked the venue and concluded the
// listing does not correspond to a real place (a fully fabricated venue, or one
// whose identity — name/entity — is wrong enough that the entry should be
// deleted rather than repaired). The classic case: the ungrounded "collection"
// populator that minted venue rows with templated descriptions and AI-guessed
// websites (2026-03-31 batch). Recommends DELETE.
//
// Because the bulk sweep cannot recompute this, sweep-gate-check.mjs carries a
// hallucinated_venue finding across re-scans (like the on-demand *_ai codes) so
// a re-sweep never silently erases a curated hallucination flag.

export const HALLUCINATION_CODE = 'hallucinated_venue'
export const HALLUCINATION_VERIFY_CODE = 'hallucinated_venue_verify'

/**
 * Build a gate6 hallucination failure descriptor for the queue.
 *
 * Two tiers, mirroring the character gate's confident/verify duality:
 *   • confident (default) — code 'hallucinated_venue', severity 3 → DELETE. Use
 *     when verification is confident no real venue matches (a fabricated entry).
 *   • verify — code 'hallucinated_venue_verify', severity 2 → PASS (keep, but
 *     surfaced for review). Use when the exact-named venue can't be confirmed but
 *     a similarly-named real venue may exist (a likely MIS-NAMED real listing that
 *     should be renamed, not deleted — the Geelong Arts Centre pattern). Never
 *     auto-recommends deletion, because a false positive would delete a real place.
 *
 * @param {string} reason - human-readable why (what the verification found)
 * @param {object} [opts]
 * @param {boolean} [opts.confident=true] - false → the verify (keep) tier
 * @returns failure descriptor
 */
export function hallucinationFailure(reason, opts = {}) {
  const confident = opts.confident !== false
  return {
    gate: 'gate6_hallucination',
    code: confident ? HALLUCINATION_CODE : HALLUCINATION_VERIFY_CODE,
    severity: confident ? 3 : 2,
    reason: reason || (confident
      ? 'Automated verification could not find any real venue matching this listing — it appears to be a fabricated / hallucinated entry.'
      : 'Automated verification could not confirm a venue by this exact name; a similarly-named real venue may exist — verify (and rename) before any deletion.'),
  }
}
...
const CODE_ACTION = {
  hallucinated_venue: 'delete',        // fabricated / mis-identified venue — remove it
  hallucinated_venue_verify: 'pass',   // can't confirm the venue — keep, but review (may be a mis-named real place)
  null_coords: 'hide',
...
// Human labels for the gates (shared by the UI).
export const GATE_LABELS = {
  ...
  gate6_hallucination: 'Hallucination',
}
