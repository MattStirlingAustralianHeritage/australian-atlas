...
const FAB_STRONG = [
  ...
  { re: /\bexhibition\s+(?:build|builds|stands?|fabricat|joinery|fit[-\s]?out)/i,                  tag: 'exhibition build' },
...
const FAB_SERVICE = [
  { re: /\bfabricat(?:ion|e|ors?|ing)\b/i,             tag: 'fabrication' },
...
export function checkGate5ServiceBusiness(listing, site = {}) {
  ...
  // "shop" arm that would spuriously trip the generic venue words. Confident
  // (hide) only on ≥2 near-unique STRONG terms, or 1 STRONG + ≥2 corroborating
  // SERVICE terms; otherwise a low-confidence verify. Runs BEFORE the generic
  // venue veto so a fabricator's client/shop chrome can't suppress it.
  const fabStrong = FAB_STRONG.filter(s => s.re.test(combined))
  const fabService = FAB_SERVICE.filter(s => s.re.test(combined))
  if (fabStrong.length >= 1 || fabService.length >= 3) {
    let visit = 0
