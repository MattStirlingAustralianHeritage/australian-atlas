...
export function getRemediations(row, listing = {}) {
  ...
  if (geo) {
    out.push({
      ...
      label: 'Re-pin from the address',
      hint: geo.code === 'wrong_state'
        ? 'Re-derive coordinates from the address. The flag clears only if the new pin lands in the listed state; otherwise the pin still moves and the flag stays for you to check.'
        : 'Re-derive coordinates from the address so the pin lands in the right place.',
    })
  }
