 ...
 *   • fix_state       — the OPPOSITE fix to regeocode, for a wrong-state pin the
 *                       reviewer judges to be CORRECT: keep the coordinates and
 *                       instead correct the mislabelled `state` (and re-derive the
 *                       region from the pin). Use it when the pin is right but the
 *                       state column is wrong — e.g. a Gold-Coast gallery tagged
 *                       NSW. Manual-only (never auto-applied): it trusts the pin,
 *                       which is the exact opposite assumption to regeocode, so
 *                       the reviewer must pick a side.
...
export function getRemediations(row, listing = {}) {
  ...
  if (geo) {
    ...
    // For a wrong-state pin, also offer the OPPOSITE fix: trust the pin and
    // correct the mislabelled state + region to match where it actually sits.
    // Manual-only (auto:false) — it's the reverse assumption to regeocode, so the
    // two must never both fire in the one-click auto-repair.
    if (geo.code === 'wrong_state') {
      out.push({
        type: 'fix_state', gates: ['gate2_location'], destructive: false, auto: false,
        label: 'Trust the pin — fix the state & region',
        hint: 'Keep the coordinates and update the listing’s state (and its region) to match where the pin actually is. Use this when the pin is right and the state label is what’s wrong.',
      })
    }
  }
...
// shortcut) is allowed to apply: the safe, non-destructive ones only, minus any
// flagged auto:false. A destructive removal must always be chosen on its own
// button; a fix that trusts a specific interpretation of an ambiguous failure
// (fix_state, which assumes the pin is right) must be an explicit reviewer choice
// so it never runs alongside its opposite (regeocode).
export function getAutoRemediations(row, listing) {
  return getRemediations(row, listing).filter(r => !r.destructive && r.auto !== false)
}
