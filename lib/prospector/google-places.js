...
async function placesBudgetReserve() {
  try {
    const { getSupabaseAdmin } = await import('@/lib/supabase/clients')
    const { reserve, PLACES_CALL_USD } = await import('@/lib/budget/governor')
    return await reserve(getSupabaseAdmin(), 'google_places', PLACES_CALL_USD)
  } catch {
...
export async function getPlaceDetails(placeId) {
  ...
  const fields = [
    ...
    // Opening hours ride the SAME Place Details call as the rating. `rating`
    // already forces the Atmosphere tier, so adding opening_hours (Contact-tier)
    // is $0 incremental — no second billed call.
