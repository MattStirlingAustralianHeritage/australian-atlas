...
export async function runPipeline(candidate, supabase, options = {}) {
  ...
  const writeResult = dryRun
    ? { id: null, errorCode: null, errorMessage: null }
    : await writeCandidate(supabase, candidate, score, websiteUrl, gateResultsSummary)

  return {
    passed: true,
    inserted: !dryRun && writeResult.id !== null,
    candidateId: writeResult.id,
    insertError: writeResult.errorCode
      ? { code: writeResult.errorCode, message: writeResult.errorMessage }
      : null,
    gateResults,
    failedGate: null,
    failReason: null,
...
const SOURCE_MAP = {
  ...
  osm_overpass: 'automated_discovery',
...
async function writeCandidate(supabase, candidate, score, websiteUrl, gateResults) {
  ...
  const row = {
    ...
    pipeline_stage: score >= 70 ? 'prepare' : 'curate',
  ...
  if (candidate.id) {
    ...
    if (error) {
      ...
      return { id: null, errorCode: error.code || 'UPDATE_FAILED', errorMessage: error.message }
    }
    return { id: candidate.id, errorCode: null, errorMessage: null }
  }
  ...
  if (error) {
    if (error.code !== '23505') {
      console.error(`[pipeline] Failed to insert candidate "${candidate.name}" [${error.code}]:`, error.message)
    }
    return { id: null, errorCode: error.code || 'INSERT_FAILED', errorMessage: error.message }
  }

  return { id: data?.id || null, errorCode: null, errorMessage: null }
}
