...
const VERTICAL_LABELS = {
  sba: 'Small Batch Atlas — craft alcohol producers only (breweries, wineries, distilleries, cideries, meaderies) with a physical production facility. The primary product must be an alcoholic beverage. NOT food producers, NOT olive oil/vinegar/honey/dairy/condiment makers, NOT cafes, NOT restaurants, NOT bars that don\'t produce alcohol, NOT non-alcoholic beverage makers, NOT importers/distributors.',
  collection: 'Culture Atlas — art museums, public galleries, science centres, discovery centres, curated private collections, sculpture parks, art archives, cultural heritage collections, natural history museums, independent cinemas, independent drive-ins, independent live music venues (dedicated band rooms, music clubs, jazz and listening rooms), independent comedy clubs (dedicated stand-up rooms). NOT tourism info centres, NOT commercial theme parks, NOT chain cinemas (Hoyts, Village, Dendy, Palace, Reading, EVT/Event Cinemas), NOT general performing arts theatres or civic/multi-purpose playhouses whose primary programme is theatre or dance (a dedicated live-music room or comedy club IS in scope), NOT venues where the cultural offering is incidental to the primary purpose (pokies pubs and sports bars that occasionally host bands).',
  ...
  table: 'Table Atlas — independent restaurants, cafes, bakeries, markets, farm gates, artisan food producers, destination dining venues. Cafes belong here if the food, space, or all-day dining experience is the primary draw (not primarily coffee). NOT chain restaurants, NOT fast food, NOT catering-only, NOT pubs without notable food. NOT cafes where coffee is the sole reason to visit (those belong in Fine Grounds).',
  way: 'Way Atlas — independently-operated Australian experience operators where the operator is the product (the traveller is buying access to a person, a method, or a route, not a building). Includes multi-day guided walks, single-day specialist guides, scenic flight and helicopter operators, sailing/sea kayak/dive operators, fishing guides, photography expeditions, specialist natural history (birding, botany, mycology, geology), foraging and bush food guides, heritage tour operators (lighthouse, mine, woolshed), multi-day workshop intensives (boat building, traditional cooking), river/canoe operators, horseback expeditions, owner-operator 4WD expeditions. Aboriginal-owned/led and Aboriginal-partnership cultural tours belong here. NOT group-operator brands (Experience Co, Journey Beyond, AAT Kings, APT, Intrepid, G Adventures, SeaLink, Discovery Holiday Parks). NOT white-label resellers. NOT bookable activities at venues already on other verticals (winery tours at SBA listings stay there). NOT free public-access trails (those are Field Atlas). NOT self-guided audio tours. NOT cooking schools at fixed venues (those are Table Atlas).',
}
...
// Map common city/suburb/region names to their state — used when
// the region string doesn't contain an explicit state code
const REGION_STATE_MAP = {
  ...
  'bathurst': 'NSW', 'lismore': 'NSW', 'mudgee': 'NSW', 'dubbo': 'NSW',
  ...
  'footscray': 'VIC', 'northcote': 'VIC', 'preston': 'VIC', 'abbotsford': 'VIC',
  'prahran': 'VIC', 'south yarra': 'VIC', 'windsor': 'VIC', 'seddon': 'VIC',
...
/**
 * Resolve expected state from a region string.
 * Tries: 1) explicit state code in string  2) city/region name mapping
 */
function resolveExpectedState(region) {
  if (!region) return null
  // 1. Explicit state code
  const match = region.match(/\b(NSW|VIC|QLD|SA|WA|TAS|ACT|NT)\b/i)
  if (match) return match[1].toUpperCase()
  // 2. City/region name mapping
  const lower = region.toLowerCase().trim()
  for (const [key, state] of Object.entries(REGION_STATE_MAP)) {
...
/**
 * Extract the state from a Mapbox geocoding response feature's context array.
 * Returns the state abbreviation (e.g., "VIC") or null.
 */
function extractStateFromMapboxContext(feature) {
  if (!feature?.context) return null
  const regionCtx = feature.context.find(c => c.id?.startsWith('region.'))
  if (!regionCtx) return null
  // short_code is like "AU-VIC"
  if (regionCtx.short_code) {
    const code = regionCtx.short_code.replace(/^AU-/, '').toUpperCase()
    if (STATE_BOUNDS[code]) return code
  }
  // Fallback: match the text (e.g., "Victoria" → "VIC")
  const stateNameMap = {
    'new south wales': 'NSW', 'victoria': 'VIC', 'queensland': 'QLD',
    'south australia': 'SA', 'western australia': 'WA', 'tasmania': 'TAS',
    'australian capital territory': 'ACT', 'northern territory': 'NT',
  }
  const text = (regionCtx.text || '').toLowerCase()
  return stateNameMap[text] || null
}
...
export async function gate2AddressRegion(candidate, websiteText, options = {}) {
  // ── Trusted-coordinate fast path ──────────────────────────────
  // If the caller supplies authoritative coordinates (e.g. OSM/Overpass or
  // Google Places, which carry the venue's real geometry), verify THOSE
  // directly instead of re-geocoding the business name. A bare-name geocode has
  // no location bias and routinely resolves generic names to the wrong state
  // ("Gutsy Sourdough, Bright VIC" → SA), false-rejecting real in-state venues.
  // Trusting good coords is both more correct and avoids that failure mode.
  const provLat = Number(candidate.lat)
  const provLng = Number(candidate.lng)
  if (candidate.lat != null && candidate.lng != null && Number.isFinite(provLat) && Number.isFinite(provLng)) {
    const au = AUSTRALIA_BOUNDS
    if (provLat < au.minLat || provLat > au.maxLat || provLng < au.minLng || provLng > au.maxLng) {
      return {
        pass: false,
        reason: `Coordinates resolve outside Australia (${provLat.toFixed(4)}, ${provLng.toFixed(4)})`,
        details: { lat: provLat, lng: provLng, geocodeConfidence: 'outside_australia' },
      }
    }
    const expectedState = resolveExpectedState(candidate.region)
    if (expectedState && STATE_BOUNDS[expectedState]) {
      const b = STATE_BOUNDS[expectedState]
      if (provLat < b.minLat || provLat > b.maxLat || provLng < b.minLng || provLng > b.maxLng) {
        let actualState = null
        for (const [state, bounds] of Object.entries(STATE_BOUNDS)) {
          if (provLat >= bounds.minLat && provLat <= bounds.maxLat && provLng >= bounds.minLng && provLng <= bounds.maxLng) {
            actualState = state
            break
          }
        }
        return {
          pass: false,
          reason: `Coordinates resolve to ${actualState || 'unknown location'}, not ${expectedState} (${candidate.region})`,
          details: { lat: provLat, lng: provLng, expectedState, actualState, geocodeConfidence: 'wrong_region' },
        }
      }
    }
    return {
      pass: true,
      details: { lat: provLat, lng: provLng, expectedState, placeName: candidate.region || null, geocodeConfidence: 'provided_coords' },
    }
  }
  ...
  try {
    const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(query)}.json?country=au&limit=1&types=poi,address,place,locality&access_token=${mapboxToken}`
    ...
    // Determine expected state from candidate region:
    //   1) Explicit state code in region string (e.g., "Barossa Valley, SA")
    //   2) Known city/region name mapping (e.g., "Melbourne" → VIC, "Carlton" → VIC)
    const expectedState = resolveExpectedState(candidate.region)

    // Also extract the actual state from the Mapbox response context
    const mapboxState = extractStateFromMapboxContext(feature)
    ...
    if (lat < auBounds.minLat || lat > auBounds.maxLat || lng < auBounds.minLng || lng > auBounds.maxLng) {
      return {
        ...
        details: { query, lat, lng, relevance, geocodeConfidence: 'outside_australia', expectedState, mapboxState },
      }
    }

    // STATE VERIFICATION — two-layer check:
    // Layer 1: If we know the expected state, verify coordinates fall within that state's bounding box
    // Layer 2: Cross-check the Mapbox response's state against the expected state
    if (expectedState) {
      // Layer 1: Bounding box check
      if (STATE_BOUNDS[expectedState]) {
        const b = STATE_BOUNDS[expectedState]
        if (lat < b.minLat || lat > b.maxLat || lng < b.minLng || lng > b.maxLng) {
          ...
          return {
            ...
            details: { query, lat, lng, relevance, expectedState, actualState, mapboxState, geocodeConfidence: 'wrong_region' },
          }
        }
      }

      // Layer 2: Mapbox response state check — catches cases where bounding boxes overlap
      // or coordinates are near borders
      if (mapboxState && mapboxState !== expectedState) {
        return {
          pass: false,
          reason: `Mapbox places this in ${mapboxState}, not ${expectedState} (${candidate.region})`,
          details: { query, lat, lng, relevance, expectedState, actualState: mapboxState, mapboxState, geocodeConfidence: 'wrong_region' },
        }
      }
    }
...
export async function gate4VerticalFit(candidate, websiteText, options = {}) {
  ...
  // SBA gets a tighter, alcohol-specific prompt to prevent food producer bleed
  const sbaExtraRules = candidate.vertical === 'sba' ? `
CRITICAL SBA RULES — Small Batch Atlas lists ONLY Australian craft alcohol producers:
- Valid types: brewery, winery, distillery, cidery, meadery
- The producer must make an ALCOHOLIC BEVERAGE as their PRIMARY product
- The producer must have a physical production facility in Australia

DISQUALIFY if ANY of the following are true:
- Primary product is non-alcoholic (olive oil, vinegar, honey, juice, kombucha, coffee, tea, smallgoods, dairy, condiments, snacks, chocolate, preserves, sauces)
- Business is primarily a restaurant, cafe, bar, or retailer that doesn't produce alcohol
- Business is a food producer that also makes wine/beer as a side product — primary product must be alcohol
- No physical production facility (online only, importer, distributor)
- Producer is international with no Australian production facility
- Business name or website indicates artisan food production rather than alcohol production
` : ''

  // Table Atlas gets cafe classification guidance
  const tableExtraRules = candidate.vertical === 'table' ? `
TABLE ATLAS CAFE RULES — A cafe belongs in Table Atlas if ANY of the following are true:
- Known for a signature food offer (house-made pastries, notable breakfast/brunch, destination-worthy kitchen)
- Operates as a bakery-cafe or produces its own baked goods
- The space or all-day dining experience is the primary draw
- Featured in food media for its food, not its coffee
- Located at a farm, winery, or produce destination

A cafe does NOT belong in Table Atlas if:
- Coffee is the sole or primary reason to visit (belongs in Fine Grounds)
- There is no meaningful food offer beyond basic snacks
- It is a specialty coffee roaster with a basic cafe attached (belongs in Fine Grounds)

When classifying cafes, set "type" to "cafe". Dual-listing with Fine Grounds is acceptable if the venue genuinely excels at both food AND coffee.
` : ''
...
Return ONLY valid JSON, no markdown fences.`
  ...
  try {
    ...
    try {
      ...
      _resv = await reserveAnthropicBudget({ model: 'claude-haiku-4-5-20251001', inputTokens: estimateTokens(prompt), maxOutputTokens: 300 })
    } catch { _resv = { ok: true } }
    if (!_resv.ok) {
      // Monthly Anthropic budget reached — skip the Claude step and continue
      // with the non-AI path (no vertical-fit enrichment available).
      console.warn('[gate4VerticalFit] anthropic monthly budget reached — skipping')
      return {
        pass: false,
        reason: 'AI budget reached — vertical fit could not be verified this month',
        details: { warning: 'anthropic monthly budget reached', confidence: null },
      }
    }
    ...
    try { const { reconcileAnthropicBudget } = await import('@/lib/ai/guardedAnthropic'); await reconcileAnthropicBudget(_resv, result.usage) } catch {}
