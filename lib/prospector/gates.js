...
const VERTICAL_LABELS = {
  ...
  collection: 'Collection Atlas — art museums, public galleries, curated private collections, sculpture parks, art archives, cultural heritage collections. NOT science centres, NOT visitor centres, NOT tourism info centres, NOT government facilities, NOT theme parks, NOT venues where collections are incidental.',
...
export async function gate0Dedup(candidate, supabase) {
  ...
  return { pass: true, details: { matchType: 'none' } }
}
...
export async function gate1WebPresence(candidate) {
  ...
  // If no URL provided, try constructing one from the business name
  if (!urlToCheck) {
  ...
  const pageText = htmlToText(html).toLowerCase()
  ...
  return {
    ...
    websiteText: htmlToText(html).slice(0, 8000),
    lastModified,
  }
}
