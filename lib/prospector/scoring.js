...
export function calculateScore(gateResults) {
  ...
  // Uses google_places_data passed through from the discovery source
  const gate0 = gateResults.gate0?.details || {}
  if (gate0.googlePlacesStatus === 'OPERATIONAL') {
    score += 10
    breakdown.googlePlaces = 10
  } else if (gate0.googlePlacesRating && gate0.googlePlacesRating >= 4.0) {
    // Partial credit for high-rated businesses even without explicit status
    score += 5
    breakdown.googlePlaces = 5
  }
