...
export async function runStage2EditorialPress(ctx, _supabase) {
  ...
  for (const hit of result.hits) {
    if (!hit.url_resolved) {
      // Per Q1 sign-off — drop unresolved URLs. We could persist them
      // for audit, but the master prompt's spirit is "don't add junk to
      // the signal record." Stage 6 silence will handle the absence.
    ...
    signals.push(buildSignal({
      ...
      sourceUrl:    hit.url,
      sourceExcerpt: hit.title || null,    // web_search doesn't return body excerpts; the article URL is the source-of-truth
      ...
      rawData: {
        ...
        title:           hit.title,
        // Persist the web_search-encoded content opaquely so a future
        // Tavily/Brave swap can re-read raw responses if needed.
        encrypted_content: hit.encrypted_content,
        search_query:    userQuery,
        // Persist the full API response only on the FIRST signal of
        // the run to avoid duplication. Detect at signal-bulk-insert
        // time? Simpler: persist on every signal but compressed via
        // raw_response_size_bytes only.
