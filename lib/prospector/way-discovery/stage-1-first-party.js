...
export async function runStage1FirstParty(ctx, _supabase) {
  ...
  // The grounding rule: every claim must trace to a text fragment
  // in the website. We require source_excerpt to actually appear
  // in extractionInput (case-insensitive substring). Signals
  // without a traceable excerpt are dropped.
  ...
  for (const claim of (extracted.claims || [])) {
    ...
    // Allow loose match: claim's excerpt should appear (potentially
    // with minor whitespace differences) in source.
  ...
  for (const claim of validated) {
    if (!claim.signal_type || !SIGNAL_TYPES.STAGE_1[Object.keys(SIGNAL_TYPES.STAGE_1).find(k =>
      ...
      // Unknown signal type — drop. Catalogue is closed at Stage 1.
      continue
    }
    ...
    signals.push(buildSignal({
      ...
      confidence,
      urlResolved:  true,                  // pages were validated by polite-fetch above
  ...
  "claims": [
    {
      ...
      "field": "<short tag like 'duration' | 'guide_name' | 'country' | 'community' >",
      "value": "<concise value, e.g. '4 days', 'Brendan Maher', 'palawa Country'>",
      "claim_text": "<one-sentence editorial-grade claim, e.g. 'wukalina Walk runs over 4 days on palawa Country.'>",
      "source_excerpt": "<verbatim text fragment from input>",
      "source_path": "<URL path like '/about', '/our-guides' if identifiable, else null>"
    }
  ]
}`
...
Extract structured editorial signals.`
