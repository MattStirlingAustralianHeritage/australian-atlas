...
// Page-text fetch budget for Stage 3 verification. Shorter than
// Stage 1's first-party budget — we just need enough to confirm
// the operator name is mentioned, not to extract claims from the page.
...
export async function runStage3Institutional(ctx, _supabase) {
  ...
  // Resolve the candidate's name variants. Phase 2B addendum (post-
  // wukalina-Walk-calibration): generic site-scoped search returns
  // body marketing pages that pass URL validation but don't actually
  // mention the operator. The variant-aware verifier below requires
  // the operator name (or a variant) to appear in the fetched page
  // text before the signal can persist. If candidate.name_variants
  // is missing (older candidates pre-migration-122), regenerate on
  // the fly so the run isn't dependent on a backfill having
  // happened.
  ...
  if (variants.length === 0) {
    log(3, `no name variants for "${candidate.name}"; skipping stage 3`)
    return []
  }
  log(3, `verifying with variants: ${JSON.stringify(variants)}`)
  ...
  for (const body of INSTITUTIONAL_BODIES) {
    ...
    for (const hit of result.hits) {
      ...
      // ─── Variant-aware operator-mention verification ─────────
      // Without this, generic body pages (e.g. ecotourism.org.au's
      // own ECO Certification info page) pass URL validation +
      // domain whitelist but aren't actually about the operator.
      // The wukalina Walk run surfaced this as a Stage 3 false-
      // positive class. Fix per Q2 sign-off addendum.
      const verification = await verifyOperatorMentioned(hit.url, variants)
      if (!verification.matched) {
        log(3, `${body.label}: dropped ${hit.url} — operator not mentioned in page text (${verification.reason})`)
        droppedNoMention++
        continue
      }

      // Confidence band logic: drop one band if only the shortest
      // variant matched (more permissive — could be a place-name
      // false positive rather than the operator). Body's default
      // band stays put for longer-variant matches.
      let confidence = body.confidence
      const onlyShort = isShortestOnlyMatch(verification.matchedVariant, variants)
      if (onlyShort) {
        confidence = (body.confidence === CONFIDENCE.HIGH)   ? CONFIDENCE.MEDIUM
                  : (body.confidence === CONFIDENCE.MEDIUM) ? CONFIDENCE.LOW
                  : CONFIDENCE.LOW
      ...
      signals.push(buildSignal({
        ...
        rawData: {
          ...
          body_default_confidence: body.confidence,
        },
        runId,
      }))
    }
    ...
    if (droppedNoMention > 0 || bandDroppedShortMatch > 0) {
      log(3, `${body.label}: ${droppedNoMention} dropped (no operator mention), ${bandDroppedShortMatch} band-dropped (shortest variant match only)`)
    }
  }
...
// ─── Operator-mention verifier ────────────────────────────────────
// Fetches the page text and tests case-normalised against each
// variant in order (longest first). Returns the matched variant
// (so the caller can decide on confidence band) or a reason for
// rejection.

async function verifyOperatorMentioned(url, variants) {
  ...
  // Walk variants longest-first; first match wins.
