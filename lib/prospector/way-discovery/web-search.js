...
export async function webSearch(params) {
  ...
  try {
    ...
    _resv = await reserveAnthropicBudget({ model: DEFAULT_MODEL, inputTokens: estimateTokens((systemPrompt ? systemPrompt + '\n' : '') + query), maxOutputTokens: 2048 })
  } catch { _resv = { ok: true } }
  if (!_resv.ok) {
    // Monthly Anthropic budget reached — skip the web_search call and return
    // an empty hit set so the pipeline continues without AI enrichment.
    console.warn('[webSearch] anthropic monthly budget reached — skipping')
    return { hits: [], rawResponse: null, usedSearches: 0 }
  }
  ...
  try { const { reconcileAnthropicBudget } = await import('@/lib/ai/guardedAnthropic'); await reconcileAnthropicBudget(_resv, rawResponse.usage) } catch {}
