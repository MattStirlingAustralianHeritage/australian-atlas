...
export async function runWayDiscoveryPipeline(candidate, supabase, opts = {}) {
  ...
  const ctx = {
    ...
    // Stage 1 deposits per-experience records here. The orchestrator
    // persists them as way_candidate_experiences and maps experience_ref
    // → experience_id on signals before bulk insert.
    experiences: [],
  }
  ...
  // ─── Persist experiences ────────────────────────────────────────
  // Stage 1 deposits per-experience records on ctx.experiences.
  // Persist these first so we get real UUIDs, then map experience_ref
  // → experience_id on signals before bulk-inserting signals.
  const refToId = {}
  if (!dryRun && ctx.experiences.length > 0) {
    const expRows = ctx.experiences.map(e => ({
      candidate_id:    candidate.id,
      name:            e.name,
      experience_type: e.experience_type,
      duration_band:   e.duration_band || null,
      run_id:          runId,
    }))
    const { data: inserted, error: expErr } = await supabase
      ...
      .select('id, name')
    if (expErr) {
      log('persist', `WARNING: experience insert failed: ${expErr.message}`)
    } else if (inserted) {
      // Map ref → UUID. Experiences are inserted in order, so
      // inserted[i] corresponds to ctx.experiences[i].
      for (let i = 0; i < inserted.length && i < ctx.experiences.length; i++) {
        refToId[ctx.experiences[i].ref] = inserted[i].id
      }
      log('persist', `inserted ${inserted.length} experiences`)
    }
  }

  // ─── Persist signals ──────────────────────────────────────────
  if (!dryRun && allSignals.length > 0) {
    // Map experience_ref → experience_id on signals.
    for (const sig of allSignals) {
      const ref = sig.raw_data?.experience_ref
      if (ref && refToId[ref]) {
        sig.experience_id = refToId[ref]
      }
    }
