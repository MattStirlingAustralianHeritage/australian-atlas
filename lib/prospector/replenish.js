...
export async function buildDedupSets(sb) {
  ...
  // PostgREST caps a single select at 1000 rows regardless of .limit(), so a
  // one-shot select silently sees only the first 1000 listings — dedup then
  // waves through candidates that already exist beyond that page. Page through
  // the full corpus so dedup is complete (the network has ~7k listings, growing).
  const existingListings = await fetchAllRows(() => sb
    .from('listings')
  ...
  const existingCandidates = await fetchAllRows(() => sb
...
/**
 * Page through a PostgREST select in 1000-row windows until exhausted.
 * `makeQuery` is a factory returning a fresh builder each call (…select().eq());
 * .range() is applied per page. Returns all rows concatenated.
 */
