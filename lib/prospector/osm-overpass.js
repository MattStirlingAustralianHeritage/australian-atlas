...
// Verticals whose POIs legitimately lack a website (national parks, reserves,
// public gardens). For these the combined-bbox query drops the ["website"]
// requirement so real gaps aren't filtered away at the source. Kept local to
// avoid a circular import with replenish.js.
const OSM_WEBSITE_EXEMPT = new Set(['field'])

// Reverse index: "key=value" selector → vertical. Built once. If a tag is
// claimed by more than one vertical the first one in OSM_SELECTORS wins
// (selectors are effectively disjoint today; this just makes the mapping
// deterministic if that ever changes).
const SELECTOR_TO_VERTICAL = (() => {
  const map = {}
  for (const [vertical, selectors] of Object.entries(OSM_SELECTORS)) {
    for (const sel of selectors) if (!(sel in map)) map[sel] = vertical
  }
  return map
})()

/**
 * Build a bbox (south, west, north, east) from a centre point + radius.
 * Longitude degrees shrink with latitude, so the E/W span is widened by
 * 1/cos(lat). Deliberately a rough square — Overpass clips to the box and the
 * pipeline verifies real state/region from coordinates downstream.
 */
export function bboxFromCenter(lat, lng, radiusMeters) {
...
/**
 * Discover candidates across ALL verticals within one bounding box in a single
 * Overpass request. Purpose-built for the methodical town-by-town gap crawl:
 * a tight town bbox surfaces the long-tail venues that the capped per-state
 * sweep never reaches (dense metros consume its result cap first).
 *
 * One combined query (union of every vertical's selectors) is far kinder to the
 * public Overpass instances than ten per-vertical queries per town. Each element
 * is classified back to its vertical by the tag that matched, so the returned
 * candidates are ready for the same dedup + 5-gate pipeline, tagged with
 * `vertical`.
 *
 * @param {[number,number,number,number]} bbox - (south, west, north, east)
 * @param {object} opts - { maxResults, deadlineMs, log }
 * @returns {Promise<object[]>} candidates (each carries `vertical`)
 */
export async function discoverAllVerticalsInBBox(bbox, opts = {}) {
  const { maxResults = 400, deadlineMs = null, log = () => {} } = opts
  if (deadlineMs && Date.now() >= deadlineMs) return []
  const box = bbox.join(',')

  // One statement per selector. Website-required for most verticals; dropped for
  // website-exempt verticals (field) so parks/reserves/gardens still surface.
  const statements = Object.entries(SELECTOR_TO_VERTICAL).map(([sel, vertical]) => {
    const [k, v] = sel.split('=')
    const webFilter = OSM_WEBSITE_EXEMPT.has(vertical) ? '' : '["website"]'
    return `nwr["${k}"="${v}"]${webFilter}["brand"!~"."](${box});`
  }).join('')
  const ql = `[out:json][timeout:90];(${statements});out tags center ${maxResults};`
  ...
  for (const el of elements) {
    ...
    // Which selector matched → which vertical.
    const matchedSel = Object.keys(SELECTOR_TO_VERTICAL).find(sel => {
      const [k, v] = sel.split('=')
      return tags[k] === v
    })
    if (!matchedSel) continue
    const vertical = SELECTOR_TO_VERTICAL[matchedSel]

    const website = tags.website || tags['contact:website'] || tags.url || null
    if (!website && !OSM_WEBSITE_EXEMPT.has(vertical)) continue

    const key = vertical + '|' + name.toLowerCase()
    if (seen.has(key)) continue
    seen.add(key)

    const lat = el.lat ?? el.center?.lat ?? null
    const lng = el.lon ?? el.center?.lng ?? el.center?.lon ?? null
    const stateCode = lat != null && lng != null ? osmState(tags, lat, lng) : null
    ...
    candidates.push({
      ...
      website_url: website,
      source: 'osm_overpass',
      source_detail: `OpenStreetMap — ${matchedSel}`,
      notes: [`OSM ${el.type}/${el.id}`, tags['opening_hours'] ? 'Has opening hours' : null].filter(Boolean).join('. '),
      status: 'pending',
      google_places_data: null,
      lat,
      lng,
  ...
  log(`[osm-bbox] ${elements.length} elements → ${candidates.length} named candidates across verticals`)
  return candidates
}
