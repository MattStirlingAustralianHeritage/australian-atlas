...
export const VERTICAL_CONFIG = {
  ...
  collection: {
    ...
    table: 'venues',
    // cinema + drive_in are sanctioned collection primary types (migration 120,
    // editorial decision 2026-05-05) with display labels + a DB constraint that
    // already permit them — but were never added here, so cinema/drive-in venues
    // couldn't sync from source and got typed 'museum' as a fallback. Keep in sync
    // with VERTICAL_CATEGORIES.collection in lib/sync/pushToVertical.js.
    typeFilter: ['archive', 'cultural_centre', 'gallery', 'botanical_garden', 'heritage_site', 'museum', 'sculpture_park', 'cinema', 'drive_in'],
  ...
  way: {
    url: process.env.WAY_SUPABASE_URL,
    serviceKey: process.env.WAY_SUPABASE_SERVICE_KEY,
    table: 'operators',
    baseUrl: 'https://wayatlas.com.au',
    listingPath: '/operators',
  },
}
...
export const VERTICAL_CLAIM_FIELD = {
  ...
  // Way uses claim_status TEXT enum ('unclaimed' | 'pending' | 'claimed' | 'paid'),
  // not a boolean flag. Phase 1 ships with a neutral default that the claim flow
  // promotes to 'claimed' / 'paid' explicitly. The enum lives on operators (Way
  // project) and is mirrored in way_meta.claim_status (portal).
  way:          { claimField: 'claim_status', claimType: 'enum', claimedValue: 'claimed' },
}
...
//
// Way Atlas: claimType='enum' writes the claimedValue ('claimed') to
// the configured field (claim_status). Stripe-payment promotion can
// override the value to 'paid' via a separate code path; this helper
// is for the boolean-style claim event only.
export function buildClaimPayload(vertical, userId) {
  ...
  } else if (config.claimType === 'enum') {
    payload[config.claimField] = config.claimedValue
  } else {
...
export const VERTICAL_CLAIMS_TABLE = {
  ...
  way:    { table: 'operator_claims',entityKey: 'operator_id',nameKey: 'contact_name',  emailKey: 'contact_email' },
}
