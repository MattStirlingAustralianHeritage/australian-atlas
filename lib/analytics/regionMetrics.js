...
//
// Interim implementation: full-fetch of the scoped pageviews window + JS
// aggregation with the SAME bot filter (isBotRow) the network dashboard uses,
// mirroring commit 4ac6108. Migration 141 adds region-scoped RPCs; once applied,
// computeRegionMetrics prefers them and falls back to this JS path (so the two
// stay parity-checkable). Heavy logic lives in the pure aggregate* helpers so a
// verifier can assert on the exact code that ships.
//
...
const PV_COLS = 'id, ts, vertical, path, visitor_id, city, region, country, lat, lng'
...
export function aggregateRegionMetrics({
  ...
  const humanRegionRows = regionRows.filter((r) => !isBotRow(r))
...
export async function computeRegionMetricsRPC(sb, region, { since, limit = 10 } = {}) {
  ...
  return {
    ...
    visitorOrigin: (m.visitor_origin || []).map((o) => ({
      city: o.city, area: o.area, country: o.country, count: Number(o.count || 0),
    })),
