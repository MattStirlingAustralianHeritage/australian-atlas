...
//
// Bot classification is geo-only because pageviews has no user_agent column yet
// (supabase/migrations/141 adds is_bot/user_agent + SQL-side aggregation; it is
// applied once the portal DB password is refreshed). The heuristic mirrors 141's
// backfill so the numbers stay identical after that migration lands.
...
export async function fetchWindowRows(sb, since, { pageSize = 1000, concurrency = 8 } = {}) {
  ...
  const cols = 'ts, vertical, path, visitor_id, city, region, country, lat, lng'
  const rows = []
...
export function aggregate(rows, { vertical = null } = {}) {
  const humans = rows.filter(r => !isBotRow(r))
