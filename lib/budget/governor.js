...
// Monthly sub-caps (USD). Env-overridable; defaults sum to $29 (+$1 buffer
// under the $30 ceiling, raised from $20 on 2026-07-11). Set AI_CAP_* in
// Vercel to retune without a deploy. Allocation follows observed pressure:
// anthropic ran to $11.5 of its old $12 cap by July 11 and both Places pools
// hit their caps, while Voyage barely registers ($0.14) — but Voyage keeps its
// headroom because it is fail-CLOSED-shared between query embeddings and the
// search reranker (rerank-2.5), and starving it kills semantic search under a
// traffic spike.
export const CAPS = {
  anthropic: num(process.env.AI_CAP_ANTHROPIC_USD, 16),
  voyage: num(process.env.AI_CAP_VOYAGE_USD, 5),
  google_places: num(process.env.AI_CAP_PLACES_USD, 4),
  ...
  google_places_admin: num(process.env.AI_CAP_PLACES_ADMIN_USD, 4),
  // Monthly closure sweep (/api/cron/closure-sweep). Its own pool so the deep
  // sweep can't starve prospecting or interactive repairs — and vice versa.
  // The sweep escalates to Places only on suspicion, so $5 covers ~90 checked
  // venues a month; raise to deepen coverage of the no-website cohort.
  google_places_closure: num(process.env.AI_CAP_PLACES_CLOSURE_USD, 5),
}
