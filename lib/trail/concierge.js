...
// The three moments, in day order. `insert` says where an accepted pick lands
// in the stop list; `anchor` says where along the route to look for it; `max`
// is how far it's reasonable to reach for that moment. Regional Australia is
// sparse — a coffee or lunch "on the way" can sit ~60 km out, and a bed for
// the night ~90 km, without feeling like a detour. Proximity scoring still
// prefers the nearest, so dense areas stay tight; the distance is always shown.
const ROLES = [
  { key: 'coffee', test: isCoffee, anchor: 'start',  insert: 'start',  max: 60 },
  { key: 'lunch',  test: isLunch,  anchor: 'middle', insert: 'middle', max: 60 },
  { key: 'stay',   test: isStay,   anchor: 'end',    insert: 'end',    max: 90 },
]
 ...
 */
export function conciergeSlots({ stops, listings, taste = null }) {
