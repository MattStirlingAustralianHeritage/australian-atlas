  ...
  if (syncToVertical && listing.vertical) {
    ...
    try {
      ...
      if (hasValidSourceId) {
        ...
        // ── Path B: no vertical row yet — push a new one and link back ──
        const pushResult = await pushToVertical(listing.vertical, syncData)
        if (pushResult.success && pushResult.id) {
          // Link the new vertical row to master so future syncs use update path
          const sb = getSupabaseAdmin()
          ...
          if (linkError) {
            ...
            // Update local listing object so caller sees the new source_id
