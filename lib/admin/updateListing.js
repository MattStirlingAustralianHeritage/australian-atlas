...
export async function updateListing(id, rawUpdates, options = {}) {
  ...
  if (touchesRegion && !addressChanged) {
    ...
    if (!name) {
      ...
      const { region: resolved } = await resolveRegionParam(name)
      if (resolved?.id) {
      ...
      // Unresolvable (a draft/unknown region name) → keep the text as typed and
      // leave the FK untouched. Drafts aren't surfaced publicly, so this can't
      // change what users see; it preserves the legacy text fallback for tools.
    }
  }
