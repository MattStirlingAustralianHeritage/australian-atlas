...
import { normalisePresencePair } from '@/lib/listings/presence'
...
// The presence trio (087 + 200) is verified present in production on both
// `listings` and `listings_with_region`; the editor needs them BACK from a save
// so its Visiting switches don't reset to the pre-save state.
...
const SELECT_COLS = 'id, vertical, source_id, name, slug, description, region, state, lat, lng, website, phone, address, hero_image_url, is_claimed, is_featured, is_market, sub_type, sub_types, status, editors_pick, presence_type, presence_types, address_on_request, created_at, updated_at'
...
const ALLOWED_FIELDS = [
  ...
  // presence_types (migration 200) rides along so the scalar and the array can
  // be written together — see §4e, which keeps them from contradicting.
  'address_on_request', 'presence_type', 'presence_types', 'visitable', 'service_area',
]
...
export async function updateListing(id, rawUpdates, options = {}) {
  ...
  if (touchesRegion || touchesAddress) {
    ...
    const { data: cur } = await sb0
      .from('listings')
      .select('address, state, suburb, region, lat, lng, region_override_id, region_computed_id, region_override:regions!region_override_id(name), region_computed:regions!region_computed_id(name)')
      .eq('id', id)
      .maybeSingle()
    currentListing = cur
  }
  ...
  if (touchesRegion && !addressChanged) {
    ...
    // Editors send every field on every save, so most saves carry an
    // UNCHANGED region. When the submitted name already matches the FK-derived
    // effective region, there is nothing to resolve — skip the regions lookup
    // (a sequential round-trip on the save's critical path) and leave the FK
    // columns untouched. Only the FK names count here: a bare text match must
    // still go through resolution so legacy text regions can gain their FK.
    const fkEffectiveName = currentListing?.region_override?.name || currentListing?.region_computed?.name || ''
    if (!name) {
      // Cleared → drop the override and fall back to the pin-computed region.
      updates.region_override_id = null
      updates.region = null
    } else if (fkEffectiveName && name === fkEffectiveName) {
      updates.region = fkEffectiveName // keep the text mirror canonical
    } else {
  ...
  // ── 4e. Presence: keep the scalar and the array in step ──────
  // `presence_type` is the single value every consumer reads; `presence_types`
  // (migration 200) is the full set. A write that moved only one of them would
  // leave a listing whose scalar says "by appointment" and whose array says
  // "markets only". Callers that own the whole picture (the Listing Editor, the
  // operator dashboard, candidate review) send BOTH and we just normalise them;
  // a caller that sends only the scalar gets its array derived from what the row
  // already holds, so an existing "markets" mode survives a by-appointment edit.
  if ('presence_type' in updates) {
    let existingTypes = updates.presence_types
    if (!('presence_types' in updates)) {
      const sbP = sb0 || getSupabaseAdmin()
      const { data: curP } = await sbP
        .from('listings')
        .select('presence_types')
        .eq('id', id)
        .maybeSingle()
      existingTypes = curP?.presence_types || null
    }
    const pair = normalisePresencePair(updates.presence_type, existingTypes)
    updates.presence_type = pair.presence_type
    updates.presence_types = pair.presence_types
  }
  ...
  if (syncToVertical && listing.vertical) {
    verticalSync = await syncListingRowToVertical(listing, { action })
  }
...
/**
 * Push an already-updated master listing row to its vertical source DB.
 *
 * Extracted from updateListing so callers can DEFER the (slow, cross-project)
 * vertical sync: the editors save master-only for an instant response, then
 * call POST /api/admin/listings/[id]/sync-vertical which runs this. Inline
 * callers (updateListing with syncToVertical=true) behave exactly as before.
 *
 * `listing` must be the CURRENT master row in SELECT_COLS shape; on paths that
 * link a vertical row, its source_id is updated in place so the caller's
 * response reflects the linkage.
 *
 * Three paths:
 ...
 *
 * Never throws — returns { success, vertical, method?, warning? } or null when
 * the listing has no vertical.
 */
export async function syncListingRowToVertical(listing, { action = 'edit' } = {}) {
  if (!listing?.vertical) return null
  const id = listing.id
  const sb = getSupabaseAdmin()
