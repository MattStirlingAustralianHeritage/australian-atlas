...
export async function updateListing(id, rawUpdates, options = {}) {
  ...
  if ('address' in updates && updates.address) {
    ...
    const { data: cur } = await sb0.from('listings').select('address, state, suburb, region, lat, lng').eq('id', id).maybeSingle()
    currentListing = cur

    if (cur && updates.address !== cur.address) {
      const state = updates.state || cur.state || null
      const suburb = updates.suburb || cur.suburb || null
      ...
      if (coords) {
        ...
        // ── 4c. Region re-assignment from new coordinates ──
        // The listings trigger (migration 097) recomputes region_computed_id by
        // polygon containment on the lat/lng write. resolveRegionForCoords adds
        // a nearest-centre fallback for points outside every polygon. We write:
        //   - region (text): kept in sync for the vertical push (verticals still
        //     read the legacy text column).
        //   - region_override_id (FK): set ONLY when the point is uncovered by
        //     any polygon, so the master detail page — which reads the FK, not
        //     the text column — always shows a region. A clean polygon hit needs
        //     no override; the trigger's computed FK is authoritative.
        try {
