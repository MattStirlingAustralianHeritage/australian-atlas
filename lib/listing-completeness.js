...
export function getCompleteness(listing, overrides = {}) {
  ...
  // "By appointment" is a complete answer to when you can visit, so a studio
  // that has said so isn't missing its opening hours — nagging it to add a
  // weekly grid it doesn't keep is asking for a fiction.
  const byAppointment = isByAppointment(source)
  const checks = COMPLETENESS_CHECKS.map(c => ({
    ...c,
    complete: c.field === 'hours'
      ? (byAppointment || !!source.hours)
      : !!(typeof source[c.field] === 'string' ? source[c.field].trim() : source[c.field]),
  }))
