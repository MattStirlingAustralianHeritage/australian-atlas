...
export const ACTIVITY_ACTIONS = {
  ...
  // How a visitor gets in the door: by appointment, address on request, and
  // (craft) whether they run classes. Same class of fact as opening hours.
  VISITING_UPDATED: 'visiting_updated',
  // Words
  ...
  // We wrote to the operator about a claim we granted without checking the
  // address was theirs. Recorded on the listing so the apology is part of its
  // history, not just a row in an admin console.
  CLAIM_REMEDIATION_SENT: 'claim_remediation_sent',
}
...
export function actionLabel(action) {
  const known = {
    ...
    visiting_updated: 'Updated how visitors get in',
    ...
    claim_remediation_sent: 'Remediation email sent to the claimant',
...
export function actionGroup(action) {
  ...
  if (/^(hours|phone|website|visiting)/.test(action)) return 'facts'
