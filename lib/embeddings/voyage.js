...
// Budget gate (lazy import keeps this file usable in raw-node scripts where the
// '@/' alias doesn't resolve — those rare admin backfills fail-open).
async function voyageBudgetReserve(inputs) {
  try {
    ...
    const estTokens = inputs.reduce((s, t) => s + Math.ceil((t?.length || 0) / 4), 0)
    const sb = getSupabaseAdmin()
    const estCost = estimateVoyageCost(estTokens)
    const ok = await reserve(sb, 'voyage', estCost)
    return { ok, sb, estCost }
  } catch {
    return { ok: true, sb: null, estCost: 0 } // governor unavailable (raw node) → don't block
  }
}
...
async function callVoyage(inputs, inputType, opts = {}) {
  ...
  // Enforce the monthly Voyage budget before spending.
  const budget = await voyageBudgetReserve(inputs)
  if (!budget.ok) throw new Error('AI_BUDGET_EXCEEDED: voyage monthly cap reached')
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    // Reconcile the reservation with actual token usage (best-effort).
    if (budget.sb && data?.usage?.total_tokens) {
      try {
        const { estimateVoyageCost, reconcile } = await import('@/lib/budget/governor')
        await reconcile(budget.sb, 'voyage', estimateVoyageCost(data.usage.total_tokens) - budget.estCost)
      } catch { /* best-effort */ }
    }
    return out
  }
...
/**
 * Embed content/documents. Default profile is PATIENT (batch backfill / cron):
 * long 429 backoff, many retries, throughput over latency. `opts` overrides the
 * retry profile per call — the operator dashboard passes a bounded profile so an
 * interactive single-listing re-embed never blocks the request (see
 * lib/embeddings/regenerateOne.js). input_type is always "document".
 */
export async function embedDocuments(texts, opts = {}) {
  const profile = { maxAttempts: 9, timeoutMs: 20000, backoff429Ms: 22000, ...opts }
  const out = []
  for (let i = 0; i < texts.length; i += MAX_BATCH) {
    out.push(...(await callVoyage(texts.slice(i, i + MAX_BATCH), 'document', profile)))
  }
  return out
}
