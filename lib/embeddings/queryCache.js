...
export async function embedQueryCached(sb, text) {
  ...
  // Write-through, fire-and-forget: the search should never wait on its own
  // cache write (it used to add a full DB round-trip before the hybrid RPC).
  try {
    sb.from('query_embedding_cache').upsert(
