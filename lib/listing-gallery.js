...
//
// Capacity is gated in the app layer to PAID listings (a live `standard`
// claim in listing_claims — active, or past_due within the dunning grace
// period; the same signal Producer's Picks uses). The store itself is
// neutral; callers enforce the gate before writing.
...
// (a live `standard` claim), never on the listings row. Kept free of any
// dependency on other PERKS, so the gallery never inherits their behaviour. The
// one import below is claim commercial state, not a perk: the comp-expiry rule
// must have exactly one definition or an expired comp keeps working in whichever
// copy is wrong.
const PAID_CLAIM_TIER = 'standard'

// 'past_due' is a DUNNING GRACE state (migration 203): a renewal payment
// failed and Stripe is retrying, but the operator was promised their Standard
// benefits stay live while they update the card. It therefore still counts as
// paid here. The grace ends when Stripe exhausts retries and fires
// customer.subscription.deleted, which drops the claim to tier 'free'
// (app/api/stripe/webhook/route.js) — so the listing stops being paid HERE,
// on the tier check below, while the operator keeps the claim itself.
const PAID_CLAIM_STATUSES = ['active', 'past_due']

// Of the given listing ids, which hold a live (active or grace-period
// past_due) standard claim. Returns a Set for O(1) membership tests. `sb` must
// be a master-portal client (listing_claims lives on master). On any query
// error the set is empty — callers fail closed.
//
// A COMPED standard claim (admin-granted, no Stripe subscription) may carry a
// term — see migration 261. This function is where that term is enforced: once
// comp_expires_at has passed the listing is unpaid HERE, immediately, whether
// or not sweepExpiredComps has yet written the tier back to 'free'. The sweep
// reconciles the stored row; this is the gate.
export async function filterPaidListingIds(sb, listingIds) {
  const ids = [...new Set((listingIds || []).filter(Boolean))]
  if (!ids.length) return new Set()
  const { data } = await sb
    .from('listing_claims')
    .select('listing_id, tier, stripe_subscription_id, comp_expires_at')
    .in('listing_id', ids)
    .in('status', PAID_CLAIM_STATUSES)
    .eq('tier', PAID_CLAIM_TIER)
  const now = new Date()
  return new Set((data || []).filter(r => !isCompLapsed(r, now)).map(r => r.listing_id))
}

// Is this single listing PAID — i.e. does it hold a live standard claim
// (active, or past_due within the dunning grace period)?
